...
import {
  ...
  ShopFloorFeatureKey,
  PendingShopFloorProcess,
  PendingShopFloorProcessStatus,
} from './types';
...
export async function createBoardIntakeQueueEntry(input: {
  ...
  quantity: number; actorName: string; actorEmail?: string; vehicleRegistration?: string; arrivalTime?: string; gate?: string;
}) {
  await execute(`INSERT INTO board_intake_queue (id, product_id, product_name, partner_id, customer_name, quantity, actor_name, actor_email, vehicle_registration, arrival_time, gate, status)
...
export async function getRecentBoardIntakeQueueEntries(limit = 12, offset = 0) {
  try {
    return await queryAll<any>(`SELECT id, product_name, customer_name, quantity, vehicle_registration, arrival_time, gate, status, error_message, retry_count, last_attempt_at, next_retry_at, created_at, synced_at, reverted_at, reverted_by, actor_name, actor_email
    ...
    return queryAll<any>(`SELECT id, product_name, customer_name, quantity, vehicle_registration, arrival_time, gate, status, error_message, retry_count, last_attempt_at, next_retry_at, created_at, synced_at, reverted_at, reverted_by, actor_name, actor_email
      FROM board_intake_queue ORDER BY created_at DESC LIMIT ? OFFSET ?`, [Math.max(1, Math.min(50, limit)), Math.max(0, offset)]);
  }
}
...
function ensureShopFloorTables() {
  ...
  execute(`ALTER TABLE board_intake_queue ADD COLUMN gate VARCHAR(50) NULL`).catch(() => {});
...
export async function getPendingShopFloorProcesses(filters?: {
  partnerId?: number;
  processType?: string;
  ...
  const rows = await queryAll<PendingShopFloorProcess>(
    `SELECT * FROM shop_floor_pending_processes ${whereClause} ORDER BY created_at DESC`,
    params,
  );
  return rows.map((r) => ({
    ...r,
...
export async function upsertPendingShopFloorProcesses(
  records: PendingShopFloorProcess[],
): Promise<void> {
  if (!records.length) return;
  const now = appDateTime();

  for (const rec of records) {
    const existing = await queryOne<{ id: string; status: string; qty_missing: number }>(
      'SELECT id, status, qty_missing FROM shop_floor_pending_processes WHERE mo_id = ? AND product_id = ?',
      [rec.mo_id, rec.product_id],
    );
    ...
    if (existing) {
      ...
      await execute(
        `UPDATE shop_floor_pending_processes
      ...
      await execute(
        `INSERT INTO shop_floor_pending_processes (
...
export async function markPendingShopFloorProcessesLoaded(filters: {
  partnerId: number;
  productId: number;
  loadedBy: string;
  quantity: number;
}): Promise<PendingShopFloorProcess[]> {
  const pendingRows = await queryAll<PendingShopFloorProcess>(
    `SELECT * FROM shop_floor_pending_processes
     WHERE partner_id = ? AND product_id = ? AND status = 'pending' AND qty_missing > 0
     ORDER BY created_at ASC`,
    [filters.partnerId, filters.productId],
  );

  let remaining = filters.quantity;
  const updated: PendingShopFloorProcess[] = [];
  const now = appDateTime();
  ...
  for (const row of pendingRows) {
    ...
    updated.push({
      ...
      status: newStatus as PendingShopFloorProcessStatus,
...
export async function deleteStaleCompletedProcesses(olderThanDays = 7): Promise<void> {
  const cutoff = appDateTimeFromNow(-olderThanDays * 24 * 60 * 60 * 1000);
  await execute(
    `DELETE FROM shop_floor_pending_processes
     WHERE status IN ('loaded', 'synced') AND updated_at < ?`,
    [cutoff],
  );
}
