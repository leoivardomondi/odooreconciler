...
import { RuntimeDatabaseConfig } from './types';
...
interface RuntimeConfigFilePayload {
  database: {
    ...
    mysqlPort?: string;
    mysqlUser?: string;
    mysqlPasswordEncrypted?: string;
    mysqlDatabase?: string;
    mysqlConnectionLimit?: string;
  };
}

const runtimeConfigPath = env.APP_RUNTIME_CONFIG_PATH;

let sqliteDb: SqliteDatabase | null = null;
let mysqlPool: Pool | null = null;
let ensuredDialect: 'sqlite' | 'mysql' | null = null;
let ensureDatabasePromise: Promise<void> | null = null;

function getDatabaseInitTimeoutMs(): number {
  const configured = Number(env.DB_INIT_TIMEOUT_MS || 30000);
  return Number.isFinite(configured) && configured > 0 ? configured : 30000;
}

async function withDatabaseOperationTimeout<T>(
  label: string,
  operation: () => Promise<T>,
  timeoutMs = getDatabaseInitTimeoutMs(),
): Promise<T> {
...
function describeDatabaseConfig(config: RuntimeDatabaseConfig): string {
...
function validateDatabaseConfig(config: RuntimeDatabaseConfig) {
  if (config.driver === 'mysql') {
    ...
    if (missing.length > 0) {
      throw new Error(
        `MySQL is selected but required environment variable(s) are missing: ${missing.join(', ')}. ` +
...
function getSqlite3Module(): {
  ...
  try {
    ...
    throw new Error(
      'SQLite mode is selected, but the sqlite3 runtime could not be loaded on this server. Use DB_CLIENT=mysql on this host or install a compatible sqlite3 build.',
      { cause: error },
    );
  }
}

function readRuntimeConfigFile(): RuntimeConfigFilePayload | null {
  if (!fs.existsSync(runtimeConfigPath)) {
    return null;
  }

  const raw = fs.readFileSync(runtimeConfigPath, 'utf8').trim();
  if (!raw) {
    return null;
  }

  return JSON.parse(raw) as RuntimeConfigFilePayload;
}

function buildRuntimeDatabaseConfig(): RuntimeDatabaseConfig {
  const fileConfig = readRuntimeConfigFile()?.database;
  const mysqlHost = env.MYSQL_HOST.trim() || fileConfig?.mysqlHost || '';
  const mysqlPort = env.MYSQL_PORT.trim() || fileConfig?.mysqlPort || '3306';
  const mysqlUser = env.MYSQL_USER.trim() || fileConfig?.mysqlUser || '';
  let mysqlPassword = env.MYSQL_PASSWORD;
  if (!mysqlPassword && fileConfig?.mysqlPasswordEncrypted) {
    try {
      mysqlPassword = decryptSecret(fileConfig.mysqlPasswordEncrypted);
    } catch (error) {
      throw new Error(
        'Stored runtime MySQL password could not be decrypted. Set DB_PASSWORD in .env/cPanel environment variables with the current password, or delete storage/runtime-config.json and restart the app.',
        { cause: error },
      );
    }
  }
  const mysqlDatabase = env.MYSQL_DATABASE.trim() || fileConfig?.mysqlDatabase || '';
  const mysqlConnectionLimit =
    env.MYSQL_CONNECTION_LIMIT.trim() || fileConfig?.mysqlConnectionLimit || '10';
  const sqlitePath = env.SQLITE_PATH || fileConfig?.sqlitePath || env.SQLITE_PATH;
  const requestedDriver = (env.DB_CLIENT || fileConfig?.driver || 'mysql').trim().toLowerCase();

  const driver: 'sqlite' | 'mysql' = requestedDriver === 'mysql' ? 'mysql' : 'sqlite';
...
export function isDatabaseInstallerEnabled(): boolean {
  return env.ENABLE_DATABASE_INSTALLER === 'true';
}

export function saveRuntimeDatabaseConfig(input: RuntimeDatabaseConfig) {
  const payload: RuntimeConfigFilePayload = {
    database: {
      driver: input.driver,
      ...
      mysqlPasswordEncrypted: input.mysqlPassword ? encryptSecret(input.mysqlPassword) : '',
  ...
  fs.writeFileSync(runtimeConfigPath, JSON.stringify(payload, null, 2), 'utf8');
  ensuredDialect = null;
  if (mysqlPool) {
    void mysqlPool.end();
    mysqlPool = null;
...
function normalizeSqliteParams(params: SqlParameters): SqliteParameter[] {
  return params.map<SqliteParameter>((value) =>
...
function sqliteExec(db: SqliteDatabase, sql: string): Promise<void> {
  return new Promise((resolve, reject) => {
    db.exec(sql, (error) => {
      if (error) {
        reject(error);
...
function sqliteAll<T extends QueryResultRow>(
  db: SqliteDatabase,
  sql: string,
...
function sqliteRun(
  ...
  return new Promise((resolve, reject) => {
    db.run(sql, normalizeSqliteParams(params), function (
...
async function getSqliteDb(config = getRuntimeDatabaseConfig()): Promise<SqliteDatabase> {
  if (!sqliteDb) {
...
async function getMysqlPool(config = getRuntimeDatabaseConfig()): Promise<Pool> {
  if (!mysqlPool) {
...
export function getDatabaseDialect(): 'sqlite' | 'mysql' {
...
async function mysqlQuery(
  ...
  return withDatabaseOperationTimeout(label, () =>
    (params ? pool.query(sql, params) : pool.query(sql)) as Promise<[unknown, unknown]>,
  );
}
...
async function mysqlExecute(
  ...
  return withDatabaseOperationTimeout(label, () =>
    (params ? pool.execute(sql, params) : pool.execute(sql)) as Promise<[unknown, unknown]>,
  );
}

async function hasColumnSqlite(
  db: SqliteDatabase,
  tableName: string,
  columnName: string,
): Promise<boolean> {
...
async function hasColumnMysql(
  ...
  const [rows] = await mysqlQuery(
    pool,
    `Checking MySQL column ${tableName}.${columnName}`,
    `
      SELECT column_name
...
async function ensureColumnMysql(
  pool: Pool,
  tableName: string,
  columnName: string,
  definition: string,
  ...
  if (!(await hasColumnMysql(pool, tableName, columnName, schemaName))) {
    await mysqlExecute(
      pool,
      `Adding MySQL column ${tableName}.${columnName}`,
      `ALTER TABLE \`${tableName}\` ADD COLUMN \`${columnName}\` ${definition}`,
    );
  }
}

async function ensureSqliteDatabase(config: RuntimeDatabaseConfig) {
...
async function ensureMysqlDatabase(config: RuntimeDatabaseConfig) {
  ...
  const query = async (label: string, sql: string) => {
...
export async function ensureDatabase() {
  ...
  ensureDatabasePromise = withDatabaseOperationTimeout(
    `Database initialization (${describeDatabaseConfig(config)})`,
    async () => {
...
export async function queryAll<T extends QueryResultRow>(sql: string, params: SqlParameters = []): Promise<T[]> {
  ...
  return sqliteAll<T>(db, sql, params);
}
...
export async function execute(sql: string, params: SqlParameters = []): Promise<ExecuteResult> {
  await ensureDatabase();
  if (getDatabaseDialect() === 'mysql') {
    const pool = await getMysqlPool();
