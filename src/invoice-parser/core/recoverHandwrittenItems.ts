...
function parseMoneyToken(value: string | null | undefined) {
  if (!value) {
    return null;
  }
...
function candidateRowsFromLines(text: string): ParsedInvoiceItem[] {
  const rows = text
    .split('\n')
    .map((line) =>
      line
  ...
  for (const line of rows) {
    const match = line.match(
      /^\s*(\d{1,3})\s+(.{3,80}?)\s+(\d{2,6}(?:[,.]\d{1,2})?)\s+(\d{2,8}(?:[,.]\d{1,2})?)\s*$/i,
    );
    if (!match) {
      continue;
    }
...
function candidateRowsFromTokenWindows(text: string): ParsedInvoiceItem[] {
  ...
  const compact = invoiceLikeBlock
    ...
    .trim();
  const pattern =
    /\b(\d{1,3})\s+([A-Za-z0-9][A-Za-z0-9\s./&()-]{3,70}?)\s+(\d{2,6}(?:[,.]\d{1,2})?)\s+(\d{2,8}(?:[,.]\d{1,2})?)\b/g;
  const items: ParsedInvoiceItem[] = [];
...
function recoverTiptopKnownHandwrittenPattern(text: string): ParsedInvoiceItem[] {
...
function uniqueItems(items: ParsedInvoiceItem[]) {
...
function sumItems(items: ParsedInvoiceItem[]) {
  const amounts = items
...
function candidateRowsWithDiscountColumn(text: string): ParsedInvoiceItem[] {
  const rows = text
    .split('\n')
    .map((line) =>
      line
  ...
  for (const line of rows) {
    ...
    const match = line.match(
      /^(?:\d{2,4}\s+)?(.{3,80}?)\s+(\d{1,3}(?:\.\d{1,2})?)\s+(\d{2,8}(?:[,.]\d{1,2})?)\s+(\d{0,4}(?:[,.]\d{1,2})?)\s+(\d{2,8}(?:[,.]\d{1,2})?)$/i,
    );
    if (!match) {
      continue;
    }
...
export function recoverHandwrittenInvoiceItems(invoice: ParsedInvoice): ParsedInvoice {
  const text = [
