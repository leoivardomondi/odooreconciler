...
import { queryAll } from '../models/db';
import { SchedulerRunEntry } from '../models/types';

export interface SchedulerFailureItem {
  id: string;
...
export interface SchedulerFailureReport {
  generatedAt: string;
  rangeLabel: string;
  fromDate: string;
  toDate: string;
  ...
  items: SchedulerFailureItem[];
}

function parseJsonContext(contextJson: string): Record<string, any> {
  try {
...
export function generateSchedulerSuggestion(
  status: string,
  summary: string,
  errorMessage: string,
  context: Record<string, any>,
): { diagnosis: string; suggestion: string } {
...
export async function getSchedulerFailureReport(scope?: {
  fromDate?: string;
  toDate?: string;
  range?: 'daily' | 'weekly' | 'all';
}): Promise<SchedulerFailureReport> {
  const now = new Date();
  let fromDate: string;
  let toDate: string;
  ...
  const rows = await queryAll<SchedulerRunRow>(
    `
      SELECT
  ...
  const items: SchedulerFailureItem[] = rows.map((r: SchedulerRunRow) => {
    if (r.status === 'failed') failedCount++;
    else if (r.status === 'skipped') skippedCount++;
    else if (r.status === 'completed_with_errors' || r.failed_count > 0) completedWithErrorsCount++;
...
export function generateSchedulerFailureCsv(report: SchedulerFailureReport): string {
  const headers = [
    'Run ID',
    'Started At',
    'Finished At',
...
export async function renderSchedulerFailurePdf(report: SchedulerFailureReport): Promise<Buffer> {
  ...
  if (report.items.length === 0) {
    ...
    report.items.forEach((item, index) => {
      if (document.y + 90 > 770) document.addPage();
      const y = document.y;
