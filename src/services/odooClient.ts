...
export class OdooClient {
   ...
   */
  async getFailedCheckouts(employeeId: number): Promise<Array<{ date: string; hours: number }>> {
    try {
      ...
      const failed: Array<{ date: string; hours: number }> = [];
      const now = new Date();
    ...
    try {
      ...
      const registeredCuttingOrders = dueCuttingOrders.filter((mo) => {
        ...
        return physicalMoves.some((move) => {
          ...
          return (
            moveDay === soDay &&
            isBoardProductName(productName) &&
