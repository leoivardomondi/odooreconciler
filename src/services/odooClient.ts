...
export class OdooClient {
  ...
  async getOpenAttendance(employeeId: number) {
    return this.request<Array<{
      id: number;
      check_in: string;
      check_out: string | null;
  ...
  async checkOutAttendance(employeeId: number) {
    ...
    if (!record) return { updated: false, record: null };
  ...
  async findMOsForBoardIntake(input: {
    ...
    try {
      if (input.partnerId === 350) return [];
      const targetCompanyId = await this.getTargetCompanyId();

      // 1. Find active MOs for target company (strictly excluding URBAN VIBE 2)
      const rawActiveMOs = await this.searchReadRecords<{
        id: number;
        name: string;
        state: string;
        origin: string | null;
