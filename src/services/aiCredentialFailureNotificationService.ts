...
function failureSignalFromLog(log: string): Omit<AiCredentialFailureSignal, 'signature'> | null {
  const isRateLimitOrTemporaryError =
    /quota|rate\s*limit|limit:\s*\d+|resource_exhausted|\b429\b|too\s+many\s+requests|retry\s+in|high\s+demand|overloaded|service\s+unavailable|\b500\b|\b502\b|\b503\b|\b504\b/i.test(
      log,
    );
