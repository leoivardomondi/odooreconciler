...
import {
  getSettings,
  hasAiCredentialFailureNotification,
  insertLog,
  recordAiCredentialFailureNotification,
} from '../models/repositories';
import { env } from '../utils/env';
import { sendMailWithConfig } from './mailTransport';

export type AiCredentialFailureKind = 'oauth_connection' | 'invalid_api_key';

export interface AiCredentialFailureSignal {
  kind: AiCredentialFailureKind;
  provider: string;
  model: string;
  message: string;
  signature: string;
...
function buildSignature(kind: AiCredentialFailureKind, provider: string, model: string, message: string) {
  return createHash('sha256')
    .update(`${kind}|${provider}|${model}|${normalizeFailureMessage(message).toLowerCase()}`)
    .digest('hex');
}

function failureSignalFromLog(log: string): Omit<AiCredentialFailureSignal, 'signature'> | null {
  const invalidApiKey =
    /api\s*key\s+(?:is\s+)?(?:not\s+valid|invalid)|invalid\s+(?:api\s*)?key|invalid\s+credential|unauthori[sz]ed|\bHTTP\s+(?:401|403)\b/i.test(log);
  const oauthFailure =
    /(?:oauth|refresh\s+token|access\s+token|google\s+gemini).*(?:failed|failure|error|invalid|expired|missing|not\s+connected|not\s+configured|unable|denied|reconnect|skipped)|(?:failed|failure|error|invalid|expired|missing|not\s+connected|not\s+configured|unable|denied|reconnect|skipped).*oauth/i.test(log);

  if (!invalidApiKey && !oauthFailure) return null;
  ...
  const kind: AiCredentialFailureKind = invalidApiKey ? 'invalid_api_key' : 'oauth_connection';
  return { kind, provider, model, message: normalizeFailureMessage(log) };
}

export function extractAiCredentialFailureSignals(logs: string[]) {
  const seen = new Set<string>();
  const signals: AiCredentialFailureSignal[] = [];
...
export async function notifyAiCredentialFailures(input: {
  attachmentId: number;
  attachmentName: string;
  logs: string[];
}) {
  const signals = extractAiCredentialFailureSignals(input.logs);
  if (signals.length === 0) return { sent: false, skipped: false, signals: [] };

  const pending = [];
  for (const signal of signals) {
    if (!(await hasAiCredentialFailureNotification(input.attachmentId, signal.signature))) {
      pending.push(signal);
    }
  }
  if (pending.length === 0) {
  ...
  for (const signal of pending) {
    await recordAiCredentialFailureNotification({
