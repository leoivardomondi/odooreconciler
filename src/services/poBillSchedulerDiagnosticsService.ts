...
export type PoBillQueueState = 'new' | 'due' | 'cooldown' | 'exhausted' | 'stable_skip' | 'processed';

export interface PoBillQueueDiagnostic {
  attachment: AttachmentInfo;
  state: PoBillQueueState;
  label: string;
  reason: string;
...
function attemptCount(pdf: AttachmentInfo) {
  return Math.max(0, Number(pdf.poBillAttemptCount || 0) || 0);
}

function reasonDetails(summaryValue: unknown) {
  const summary = String(summaryValue || '').trim();
  const lower = summary.toLowerCase();
  if (!summary) return { category: 'Not checked', reason: 'The scheduler has not checked this document yet.' };
...
function poBillRuns(runs: SchedulerRunEntry[]) {
  return runs.filter((run) => run.context.jobType === 'po_bill_matching');
}

function outcomeReasons(runs: SchedulerRunEntry[]) {
  ...
  for (const run of runs) {
    ...
    for (const rawOutcome of outcomes) {
      ...
      if (!attachmentId || reasons.has(attachmentId)) continue;
      const diagnostics = outcome.diagnostics && typeof outcome.diagnostics === 'object'
        ? outcome.diagnostics as Record<string, unknown>
        : {};
      const checks = Array.isArray(diagnostics.checks) ? diagnostics.checks : [];
      const failedChecks = checks
        .filter((rawCheck) => rawCheck && typeof rawCheck === 'object' && (rawCheck as Record<string, unknown>).status !== 'pass')
        .map((rawCheck) => {
      ...
      if (detailed) reasons.set(attachmentId, detailed);
    }
  }
  return reasons;
}
...
export async function buildPoBillSchedulerDiagnostics(documents: AttachmentInfo[]) {
  ...
  const reasonCounts = attention.reduce<Record<string, number>>((counts, item) => {
  ...
  return {
    ...
    counts: queue.reduce<Record<PoBillQueueState, number>>((counts, item) => {
      counts[item.state] += 1;
      return counts;
