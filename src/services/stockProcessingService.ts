import {
  acquireStockProcessingLock,
  getLatestExtractedResultByOrderId,
  getProcessedStockVariantIds,
  getSettings,
  getUnreversedProcessedStockItemsForOrder,
  insertProcessedStockItem,
  isStockProcessingLocked,
  markProcessedStockItemReversed,
  releaseStockProcessingLock,
} from '../models/repositories';
import {
  ParsedEdgingItem,
  SaleOrderLine,
  StockProcessingItemResult,
  StockProcessingRunResult,
} from '../models/types';
import {
  formatOdooDateTime,
...
interface StockLocationResolution {
  id: number;
  label: string;
}

interface StockComponentResolution {
  found: boolean;
...
interface EdgeBandServiceVariantMapping {
  serviceColor: string;
  serviceNames: string[];
  variantColor: string;
  variantType: string;
...
const EDGE_BAND_SERVICE_VARIANT_MAPPINGS: EdgeBandServiceVariantMapping[] = [
...
function buildExpectedSoProduct(normalizedColor: string): string {
  return `Edge Banding Service ${toTitleCase(normalizedColor)}`;
}

function normalizeEdgeBandType(thicknessMm: number | null | undefined): string {
  const thickness = Number(thicknessMm || 0);
  return thickness >= 30 ? '36mm' : '1mm';
}

function resolveEdgeBandMapping(
  normalizedColor: string,
  thicknessMm: number | null | undefined,
): EdgeBandServiceVariantMapping | null {
  const preferredType = normalizeEdgeBandType(thicknessMm);
  const matches = EDGE_BAND_SERVICE_VARIANT_MAPPINGS.filter(
    (mapping) => normalizeColor(mapping.serviceColor) === normalizedColor,
  );
  return matches.find((mapping) => mapping.variantType === preferredType) || matches[0] || null;
}

function splitColorWords(value: string): string[] {
...
function colorsAreEquivalent(left: string, right: string): boolean {
...
function extractItemsFromJson(rawJson: string): ParsedEdgingItem[] {
  const parsed = JSON.parse(rawJson) as unknown;

  if (Array.isArray(parsed)) {
    return parsed.filter((item): item is ParsedEdgingItem => Boolean(item && typeof item === 'object'));
  }

  if (parsed && typeof parsed === 'object' && Array.isArray((parsed as { items?: unknown[] }).items)) {
    return (parsed as { items: ParsedEdgingItem[] }).items;
  }
...
function shouldPreferLatestExtraction(
  handoffJson: string,
  currentSignature: string,
  latestExtracted: Awaited<ReturnType<typeof getLatestExtractedResultByOrderId>>,
): boolean {
  if (!latestExtracted) {
    return false;
  }
...
function aggregateItems(items: ParsedEdgingItem[]): AggregatedStockItem[] {
  const byColor = new Map<string, AggregatedStockItem>();
...
function normalizeServiceProductName(value: string): string {
...
function getLineDescriptionCandidates(value: string): string[] {
  return value
    .split(/\r?\n/)
    .map((part) => part.trim())
    .filter(Boolean);
...
function extractServiceColor(value: string): string | null {
...
function getLineServiceColors(line: SaleOrderLine): string[] {
  const values = [
    ...(Array.isArray(line.product_id) ? [line.product_id[1]] : []),
    ...getLineDescriptionCandidates(line.name || ''),
  ];
  return [...new Set(values.map(extractServiceColor).filter((value): value is string => Boolean(value)))];
}
...
function colorSimilarity(left: string, right: string): number {
  const normalizedLeft = normalizeColor(left);
  const normalizedRight = normalizeColor(right);
  if (!normalizedLeft || !normalizedRight) return 0;
  if (colorsAreEquivalent(normalizedLeft, normalizedRight)) return 1;
...
export function matchSoLine(item: AggregatedStockItem, lines: SaleOrderLine[]): SaleOrderLine | null {
  const expectedNames = item.serviceNameCandidates.map((candidate) => normalizeServiceProductName(candidate));

  const matchesExpectedName = (candidate: string) =>
    expectedNames.includes(normalizeServiceProductName(candidate));
  ...
  return (
    lines.find((line) =>
      getLineDescriptionCandidates(line.name || '').some(matchesExpectedName),
    ) || null
  );
}

export function matchSoLines(
  items: AggregatedStockItem[],
  lines: SaleOrderLine[],
): Map<AggregatedStockItem, SaleOrderLine> {
  const matches = new Map<AggregatedStockItem, SaleOrderLine>();
  ...
  const unmatchedLines = lines.filter(
    (line) => !usedLineIds.has(line.id) && !line.display_type && getLineServiceColors(line).length > 0,
  );
  ...
  const scores = unmatchedItems.map((item) =>
    unmatchedLines.map((line) =>
      Math.max(0, ...getLineServiceColors(line).map((color) => colorSimilarity(item.normalizedColor, color))),
    ),
  );
  const candidates: Array<{ itemIndex: number; lineIndex: number; score: number }> = [];

  scores.forEach((row, itemIndex) => {
    row.forEach((score, lineIndex) => {
      if (score >= 0.72) candidates.push({ itemIndex, lineIndex, score });
    });
  });
  ...
  for (const candidate of candidates) {
    if (usedItems.has(candidate.itemIndex) || usedLines.has(candidate.lineIndex)) continue;
    ...
    if (candidate.score - nextItemScore < 0.12 || candidate.score - nextLineScore < 0.12) continue;
...
function extractEdgeBandRollColor(value: string): string | null {
...
function extractEdgeBandRollType(value: string): string {
  const typeMatch = value.match(/\b(36|1)\s*mm\b/i);
  return typeMatch ? `${typeMatch[1]}mm` : '1mm';
}

export function computeExtractedMeters(lengthMm: number): number {
  return Math.round(Math.max(0, lengthMm) / 1000);
}

export function computeQuantityToAdd(lengthMm: number, soLineQuantity: number | null | undefined): number {
...
async function resolveStockLocation(client: OdooClient): Promise<StockLocationResolution> {
  const settings = await getSettings();
...
async function resolveComponentFromBOM(
  client: OdooClient,
  ...
  const exactVariantBoms = scopedBomCandidates.filter(
    (candidate) => Array.isArray(candidate.product_id) && candidate.product_id[0] === product.id,
  );
  const templateBoms = scopedBomCandidates.filter(
    (candidate) =>
      !Array.isArray(candidate.product_id) &&
...
async function ensureSignatureStillCurrent(
  client: OdooClient,
  orderId: number,
  signature: string,
) {
  ...
  const unreversedProcessedItems = await getProcessedStockVariantIds(orderId, signature);

  if (
    latest.signature.trim() &&
...
function appendProcessingLog(existingLog: string, nextEntry: string): string {
  return existingLog.trim() ? `${existingLog.trim()}\n\n${nextEntry}` : nextEntry;
}

function buildSummaryLog(result: StockProcessingRunResult): string {
  const timestamp = formatOdooDateTime(new Date());
  const previewLabel = result.preview ? 'Preview' : 'Process';
  const sourceLabel =
    result.source === 'stock_adjustment_input_json'
...
function createBaseItemResult(item: AggregatedStockItem): StockProcessingItemResult {
  return {
...
async function buildStockRun(orderId: number): Promise<{
  client: OdooClient;
  orderName: string;
  signature: string;
  ...
  handoffLog: string;
  items: AggregatedStockItem[];
  mappings: ReturnType<typeof resolveFieldMappings>;
}> {
  const { client, settings } = await getConfiguredClient();
  const availableFields = await client.getSaleOrderFields();
  const mappings = resolveFieldMappings(settings.fieldMappings, availableFields);
  const requiredMappings: Array<keyof typeof mappings> = [
    'signatureField',
    'stockProcessedField',
    'stockSignatureField',
    'deltaJsonField',
  ...
  const latestExtracted = await getLatestExtractedResultByOrderId(orderId);
  let source: 'stock_adjustment_input_json' | 'latest_extraction' = 'latest_extraction';
  let sourceItems: ParsedEdgingItem[] = [];
  let signature = handoff.signature.trim();
  const preferLatestExtraction = shouldPreferLatestExtraction(
    handoff.stockAdjustmentInputJson,
    signature,
    latestExtracted,
  );
...
async function resolveAllowedNotificationRecipient(
  client: OdooClient,
  kind: 'missing_so' | 'missing_component' | 'manual_missing_items',
): Promise<StockAlertRecipient | null> {
  const user = await client.findUserByNameLoginOrEmail(ALLOWED_NOTIFICATION_USER_NAME);
  if (!user) {
    await logEvent('warn', 'Allowed stock alert user could not be resolved in Odoo', {
      kind,
...
export async function processStockItem(
  client: OdooClient,
  orderId: number,
  item: AggregatedStockItem,
  lines: SaleOrderLine[],
  location: StockLocationResolution,
  signature: string,
  preview: boolean,
...
export async function processAllItems(
...
): Promise<StockProcessingRunResult> {
  const preview = Boolean(options.preview);
  const run = await buildStockRun(orderId);
  const { client } = run;
  const lockSkipped = !preview && (await isStockProcessingLocked(orderId, run.signature));
  const initialResult: StockProcessingRunResult = {
    orderId,
  ...
  const availableFields = await client.getSaleOrderFields();
  const mappings = resolveFieldMappings((await getSettings()).fieldMappings, availableFields);
  const handoff = await client.getSaleOrderStockHandoff(orderId, mappings);
  const unreversedProcessedItems = await getProcessedStockVariantIds(orderId, run.signature);

  if (
    handoff.signature.trim() &&
  ...
  const lockAcquired = preview ? true : await acquireStockProcessingLock(orderId, run.signature);
  ...
  try {
    ...
    const processedVariantIds = new Set<number>(await getProcessedStockVariantIds(orderId, run.signature));
    const matchedLines = matchSoLines(run.items, lines);
    const results: StockProcessingItemResult[] = [];
    ...
    for (const item of run.items) {
      ...
      if (!preview && result.status === 'processed' && result.variantId) {
        await insertProcessedStockItem({
          orderId,
          ...
          historyId: (await getLatestExtractedResultByOrderId(orderId))?.historyId || null,
        });
      }
    ...
    const summary = {
      ...
      missingSoItemsCount: results.filter(
        (item) => item.status === 'skipped' && item.skipReason === 'No matching Sales Order line',
      ).length,
      missingComponentCount: results.filter((item) =>
        item.skipReason.startsWith('ATTENTION: Component missing in BOM'),
      ).length,
      zeroQuantityCount: results.filter(
        (item) => item.status === 'skipped' && item.skipReason === 'No unused quantity to add',
      ).length,
    };
    const alreadyProcessedOnly =
      !preview &&
      results.length > 0 &&
      results.every(
      ...
      if (alreadyProcessedOnly) {
        ...
        for (const message of componentMissingMessages) {
          if (!missingComponentAlertRecipient?.partnerId) {
            await logEvent('warn', 'Skipped missing component chatter alert because the allowed notification user was not resolved', {
              orderId,
              allowedUser: ALLOWED_NOTIFICATION_USER_NAME,
              message,
            });
          ...
          await client.postChatterAlert(
            orderId,
            `${recipientPrefix}${message}`,
            [missingComponentAlertRecipient.partnerId],
          );
        }
    ...
    await logEvent('info', preview ? 'Stock reconciliation preview completed' : 'Stock reconciliation completed', {
      orderId,
      orderName: run.orderName,
      signature: run.signature,
      preview,
    ...
    await logEvent('error', preview ? 'Stock reconciliation preview failed' : 'Stock reconciliation failed', {
      orderId,
      signature: run.signature,
      preview,
    ...
    if (!preview) {
      await releaseStockProcessingLock(orderId, run.signature);
    }
  }
}

export async function previewSaleOrderStockProcessing(orderId: number) {
  return processAllItems(orderId, { preview: true });
}

export async function processSaleOrderStock(orderId: number) {
  return processAllItems(orderId, { preview: false });
}

export async function reverseSaleOrderStockAdditions(orderId: number): Promise<{
  orderId: number;
  ...
  if (await isStockProcessingLocked(orderId, reverseLockSignature)) {
    throw new Error('A stock reversal is already in progress for this Sales Order.');
  }

  const lockAcquired = await acquireStockProcessingLock(orderId, reverseLockSignature);
  if (!lockAcquired) {
  ...
  try {
    const location = await resolveStockLocation(client);
    const items = await getUnreversedProcessedStockItemsForOrder(orderId);

    let reversedCount = 0;
    let affectedVariants = 0;

    if (items.length > 0) {
      const grouped = new Map<number, { totalMeters: number; itemIds: string[]; colors: string[] }>();
      for (const item of items) {
      ...
      for (const [variantId, group] of grouped.entries()) {
        ...
        if (newStock < 0) {
          throw new Error(
            `Cannot reverse ${group.totalMeters}m for variant ${variantId}; current stock at ${location.label} is only ${currentStock}.`,
          );
        }

        await client.adjustStockAtLocation(variantId, location.id, newStock);
        for (const itemId of group.itemIds) {
          await markProcessedStockItemReversed(itemId, orderId);
        }
      }
    ...
    const payload: Record<string, unknown> = {
      [mappings.logField]: appendProcessingLog(
        handoff.processingLog,
        `[${formatOdooDateTime(new Date())}] Stock reversal | ${summaryMessage}`,
      ),
    ...
    await logEvent('warn', 'Stock additions reversed', {
      orderId,
      reversedCount,
      affectedVariants,
    ...
    await releaseStockProcessingLock(orderId, reverseLockSignature);
  }
}
