...
const NVIDIA_TABLE_STRUCTURE_ENDPOINT =
  process.env.NVIDIA_TABLE_STRUCTURE_ENDPOINT ||
...
function isMpesaReceiptText(value: string) {
  return /^[A-Z0-9]{10}$/i.test(value) && /[A-Z]/i.test(value);
}

function joinSplitReceiptLines(value: string) {
  return value.replace(
    /\b([A-Z0-9]{6,9})\s*\n\s*([A-Z0-9]{1,4})\s*\n\s*(20\d{2}[-/]\d{1,2}[-/]\d{1,2})\b/g,
    (match, first: string, second: string, date: string) => {
...
function prepareMpesaStatementText(value: string) {
...
function cleanPartyName(value: string | null | undefined) {
  return String(value || '')
    .replace(/\s+/g, ' ')
...
function collectBoundingBoxCounts(payload: unknown) {
  ...
  for (const item of data) {
    ...
    const boxes = (item as { bounding_boxes?: Record<string, unknown> }).bounding_boxes || {};
...
async function runNvidiaTableStructure(input: {
  ...
  const summaries: Array<{ pageNumber: number; tables: number; rows: number; columns: number; cells: number }> = [];
  ...
  for (const image of input.images) {
    try {
      ...
      if (imageB64.length > NVIDIA_DIRECT_UPLOAD_BASE64_LIMIT) {
        input.warnings.push(
          `NVIDIA table structure skipped page ${image.pageNumber} because the image exceeds direct upload size after resizing.`,
        );
        continue;
      }
      if (resized) {
      ...
      const response = await axios.post(
        NVIDIA_TABLE_STRUCTURE_ENDPOINT,
        {
          input: [
            {
              type: 'image_url',
  ...
  if (summaries.length > 0) {
    input.warnings.push(
      `NVIDIA table structure detected ${totals.cells} cell(s), ${totals.rows} row(s), ${totals.columns} column(s), and ${totals.tables} table region(s).`,
    );
  }
...
function collectDecimalMoneyValues(text: string) {
...
function collectMoneyValues(text: string) {
...
function normalizeDateMatch(value: string | null) {
  if (!value) {
    return null;
  }
...
function extractDateAndTime(row: string) {
  const match =
    row.match(DATE_TIME_PATTERN) ||
...
function classifyTransaction(details: string, paidIn: number | null, withdrawn: number | null): {
...
function extractCounterparty(details: string) {
  ...
  const partyWithPrefixMatch = details.match(
    /(?:^|\b)(?:254\d{3}\*{3}\d{3}|0[17]\d{2}\*{3}\d{3}|254\d{9}|0[17]\d{8}|\d{5,8})\s*[-:]\s*([A-Za-z0-9\s.'&-]+?)(?:\s+Completed|\s+Customer|\s+Merchant|\s+OD|\s+Payment|\s+Transfer|$)/i,
  );
...
function summarizeDetails(input: {
  rawDetails: string;
  classified: { direction: MpesaTransaction['direction']; transactionType: string };
...
function isMorningCompletionTime(value: string | null | undefined) {
  const hour = Number(String(value || '').match(/^(\d{1,2}):/)?.[1]);
  return Number.isFinite(hour) && hour >= 0 && hour < 12;
}

function isJanetAwinoOchieng(value: string | null | undefined) {
  const normalized = normalizeSearch(value);
  return normalized.includes('janet awino ochieng') || normalized.includes('janet ochieng');
}

function isGeorgeOkullo(value: string | null | undefined) {
  const normalized = normalizeSearch(value);
  return normalized.includes('george okullo');
}

function isMaifanMineralWater(value: string | null | undefined) {
  const normalized = normalizeSearch(value);
  return normalized.includes('maifan mineral water');
}

function isRiderTransport(value: string | null | undefined) {
  const normalized = normalizeSearch(value);
...
function isBankDeposit(text: string | null | undefined) {
  const normalized = normalizeSearch(text);
...
function applyMpesaBusinessRules(input: {
  ...
  if (
    input.classified.direction === 'out' &&
  ...
  if (
    ...
    if (transportAnalysis.isTransport && transportAnalysis.totalWeight >= 6) {
      const category =
        transportAnalysis.subType === 'staff_transport'
...
function cleanupDetails(row: string, receiptNumber: string | null, dateText: string) {
  let details = row;
  if (receiptNumber) {
 ...
 */
function trimTrailingTransactionOverlap(details: string): string {
  ...
  const overlapPatterns: Array<{ pattern: RegExp; label: string }> = [
...
function parseTransactionRow(
  row: string,
  rowIndex: number,
  forcedMoneyValues?: number[],
  explicitOtherPartyText?: string,
  ...
  const finalOtherPartyText =
    explicitOtherPartyText ||
...
function isReceiptLine(value: string) {
  return isMpesaReceiptText(value);
}

function parseStructuredTransactionBlock(block: string[], startRowIndex: number): ParsedMpesaTransaction[] {
  ...
  for (let ci = 0; ci < completedIndices.length; ci += 1) {
    ...
    const amountValues = collectDecimalMoneyValues(
      [afterCompleted, ...rest.slice(completedIndex + 1, completedIndex + 3)].join(' '),
    );
...
function parseStructuredTransactionsFromText(text: string) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    while (
      ...
      if (
        /\b(Organization\s+Transfer|Customer\s+Merchant\s+Payment|Pay\s*Bill|Withdraw(al)?\s+(From|At|Cash)|(Send|Sent)\s+to|Funds\s+Received)\b/i.test(lines[endIndex])
      ) {
        break;
      }
      endIndex += 1;
...
function collectPotentialRows(text: string) {
  const rows: string[] = [];
  let current = '';
  ...
  for (const line of normalizeSpaces(text).split('\n')) {
    ...
    const startsTransaction =
      new RegExp(`^${MPESA_RECEIPT_PATTERN.source}\\s*\\d{4}[-/]\\d{1,2}[-/]\\d{1,2}`, 'i').test(trimmed) ||
...
function parseEmbeddedPdfTransactions(text: string) {
  ...
  const parsed = starts
    .map((match, index) => {
      ...
      let explicitOtherParty: string | undefined = cleanTrailing || undefined;
...
function expandKnownMpesaPartyAliases(value: string | null | undefined) {
...
function findKnownMpesaPayerAlias(transactionText: string, customerText: string) {
  return KNOWN_MPESA_PAYER_ALIASES.find(
    (alias) =>
      hasAllSearchTokens(customerText, alias.customerName) &&
      alias.payerNames.some((payerName) => hasAllSearchTokens(transactionText, payerName)),
  );
}

function findKnownMpesaPayerAliasForTransaction(transactionText: string) {
  return KNOWN_MPESA_PAYER_ALIASES.find((alias) =>
    alias.payerNames.some((payerName) => hasAllSearchTokens(transactionText, payerName)),
  );
}

function mergeInvoicesById(invoices: CustomerInvoiceSummary[]) {
  return [...new Map(invoices.map((invoice) => [invoice.id, invoice])).values()];
}

function buildPaymentsByInvoiceId(payments: CustomerPaymentSummary[]) {
  const paymentsByInvoiceId = new Map<number, CustomerPaymentSummary[]>();
...
async function readCustomerInvoicesByIds(
  client: OdooClient,
  invoiceIds: number[],
  warnings: string[],
) {
  ...
  try {
    return await client.readRecords<CustomerInvoiceSummary>('account.move', uniqueIds, [
      'id',
      'name',
      'ref',
      'company_id',
...
function scorePurchaseOrderCandidate(
...
function scoreInvoiceAmount(
  ...
  ].filter((entry) => typeof entry.value === 'number' && entry.value > 0) as Array<{ value: number; label: string }>;
...
function getPaymentInvoiceIds(payment: CustomerPaymentSummary) {
...
function scoreLinkedPaymentAmount(
  transaction: ParsedMpesaTransaction,
...
function scoreCustomerInvoiceCandidate(
  transaction: ParsedMpesaTransaction,
  invoice: CustomerInvoiceSummary,
  linkedPayments: CustomerPaymentSummary[] = [],
): MpesaPurchaseOrderCandidate {
  const reasons: string[] = [];
  ...
  const bestLinkedPaymentScore = linkedPayments.reduce(
    (bestScore, payment) => Math.max(bestScore, scoreLinkedPaymentAmount(transaction, payment, reasons)),
    0,
  );
...
async function addOdooReconciliationCandidates(
  ...
  let customerPayments: CustomerPaymentSummary[] = [];
  const hasPurchasePaymentRows = transactions.some((transaction) =>
  ...
  const hasSalesReceiptRows = transactions.some((transaction) =>
    transaction.direction === 'in' && Boolean(transaction.amount) && transaction.transactionType !== 'mpesa_charge',
  );
    ...
    if (transaction.direction === 'in' && transaction.amount && transaction.transactionType !== 'mpesa_charge') {
      ...
      const candidates = [
        ...eligibleInvoices.map((invoice) =>
          scoreCustomerInvoiceCandidate(transaction, invoice, paymentsByInvoiceId.get(invoice.id) || []),
        ),
      ]
...
export async function extractMpesaStatement(input: {
  ...
  try {
    const pdfText = await extractPdfText(input.filePath);
    ...
    const nvidiaTableApiKey =
      ...
      '';
    const shouldRunNvidiaTableStructure =
      Boolean(nvidiaTableApiKey) ||
