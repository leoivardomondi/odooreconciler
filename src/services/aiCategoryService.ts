...
type MpesaCategory = (typeof MPESA_CATEGORIES)[number];

interface MpesaCategoryTrainingRule {
  id: string;
  matchScope: 'any' | 'notes' | 'details' | 'counterparty' | 'raw';
  matchText: string;
  category: MpesaCategory;
  source: 'manual' | 'ai' | 'review';
...
const CATEGORY_LABELS: Record<MpesaCategory, string> = {
...
const CATEGORY_DESCRIPTIONS: Record<MpesaCategory, string> = {
  ...
  supplier_payment: 'Payment to a supplier/vendor for Purchase Orders (POs), materials, vendor bills, paybill, till number, or merchant payment',
...
function getCategorizationText(input: CategorizationTextInput) {
...
function extractCategoryCandidatesFromText(value: string): MpesaCategory[] {
  const normalized = normalizeCategoryText(value);
  if (!normalized) {
    return [];
  }

  const candidates: MpesaCategory[] = [];
...
function scoreTrainingMatch(rule: MpesaCategoryTrainingRule, text: {
  notes: string;
  details: string;
  counterparty: string;
  rawDetails: string;
  ...
  const sources: Array<[MpesaCategoryTrainingRule['matchScope'], string]> = [
    ['notes', text.notes],
...
async function loadActiveMpesaCategoryTrainingRules(): Promise<MpesaCategoryTrainingRule[]> {
  ...
  return rows
    .map((row) => ({
      ...
      matchScope: (['any', 'notes', 'details', 'counterparty', 'raw'].includes(row.match_scope)
        ? row.match_scope
        : 'any') as MpesaCategoryTrainingRule['matchScope'],
      matchText: normalizeCategoryText(row.match_text),
      category: MPESA_CATEGORIES.includes(row.category as MpesaCategory)
        ? (row.category as MpesaCategory)
        : 'unknown',
      source: (['manual', 'ai', 'review'].includes(row.source) ? row.source : 'manual') as MpesaCategoryTrainingRule['source'],
...
function buildTrainingPhraseCandidates(input: CategorizationTextInput): Array<{
  matchScope: MpesaCategoryTrainingRule['matchScope'];
  matchText: string;
  sampleText: string;
}> {
...
async function saveMpesaCategoryTrainingRule(input: {
  ...
  matchText: string;
  sampleText: string;
  source?: MpesaCategoryTrainingRule['source'];
  confidence?: number;
}) {
  ...
  const confidence = Number.isFinite(input.confidence as number) ? Math.max(0, Math.min(1, Number(input.confidence))) : 1;
...
export async function trainMpesaCategoryFromTransaction(input: {
  category: MpesaCategory;
  details: string;
  counterparty: string | null;
  direction: string;
  ...
  const preferred =
    candidates.find((candidate) => candidate.matchScope === 'notes') ||
...
// ─── Transport Keyword System ───────────────────────────────────────────

/**
 * Comprehensive transport-related keyword patterns.
 * These are organized into tiers by signal strength.
 */

interface TransportKeywordRule {
  /** The regex pattern to match */
  pattern: RegExp;
  /** Weight contributed if matched (higher = stronger signal) */
  weight: number;
  /** Human-readable label for why this matched */
  label: string;
}

const TRANSPORT_KEYWORD_RULES: TransportKeywordRule[] = [
...
/**
 * Keyword rules for other categories (used in fallback classification)
 */
...
const OTHER_CATEGORY_KEYWORDS: CategoryKeywordRule[] = [
  {
...
function buildCategoryPrompt(input: {
  ...
  const categoryList = MPESA_CATEGORIES.map(
  ...
  return [
    ...
    '- "supplier_payment" is for paying a supplier/vendor for Purchase Orders (POs) (e.g. mentions of "PO 631", "purchase order", "LPO", or vendor till/paybill).',
...
export interface TransportKeywordAnalysis {
  /** Whether this transaction likely relates to transport */
  isTransport: boolean;
  /** Confidence score 0-1 based on keyword matches */
  confidence: number;
  /** Which keyword rules matched */
  matchedRules: Array<{ label: string; weight: number }>;
  /** Total weight score from matched rules */
  totalWeight: number;
  /** Whether this is staff transport vs goods transport */
  subType: 'staff_transport' | 'goods_transport' | 'unknown';
}
 ...
 */
export function analyzeTransportKeywords(input: {
  details: string;
  counterparty: string | null;
  rawDetails?: string;
  direction: string;
}): TransportKeywordAnalysis {
  const combinedText = [
    input.details,
    input.counterparty || '',
    input.rawDetails || '',
  ...
  const matchedRules: Array<{ label: string; weight: number }> = [];
  let totalWeight = 0;
  ...
  // Determine sub-type
  let subType: TransportKeywordAnalysis['subType'] = 'unknown';
 ...
 */
export function categorizeByKeywords(input: {
  details: string;
  counterparty: string | null;
  direction: string;
  paidIn: number | null;
...
async function resolveStrongCategorizationOverride(input: CategorizationTextInput): Promise<{
  category: MpesaCategory;
  confidence: number;
  reason: string;
} | null> {
  const text = getCategorizationText(input);
  const manualCandidates: Array<{ category: MpesaCategory; confidence: number; reason: string }> = [];
  ...
  let bestTraining: { category: MpesaCategory; confidence: number; reason: string; score: number } | null = null;
...
function extractChatOutputText(response: any): string {
...
function extractAnthropicOutputText(response: any): string {
...
function extractJsonObjectText(value: string): string {
  ...
  if (directStart < 0) return stripped;
  ...
  for (let index = directStart; index < stripped.length; index += 1) {
    ...
    if (char === '{') depth += 1;
      ...
      depth -= 1;
      if (depth === 0) return stripped.slice(directStart, index + 1);
    }
  }
  return stripped.slice(directStart);
}

function isTextOnlyNvidiaModel(model: string): boolean {
...
export interface AiCategoryResult {
  ...
  aiProvider?: string;
  aiModel?: string;
  /** Transport-specific keyword analysis (always populated) */
  transportAnalysis: TransportKeywordAnalysis;
}
...
async function tryAiCategorization(input: {
  ...
  try {
    ...
    const parsed = JSON.parse(jsonText) as { category?: unknown; confidence?: unknown; reason?: unknown };

    const category: MpesaCategory = MPESA_CATEGORIES.includes(parsed.category as MpesaCategory)
      ? (parsed.category as MpesaCategory)
      : 'unknown';
    const confidence =
...
export interface BatchCategoryInput {
  id: string;
  details: string;
  counterparty: string | null;
  direction: string;
...
export async function categorizeBatchWithAi(
  transactions: BatchCategoryInput[],
 ...
 */
export function extractTransportIndicators(input: {
  details: string;
  counterparty: string | null;
  rawDetails?: string;
}): string[] {
 ...
 */
export function getTransportKeywordRules(): TransportKeywordRule[] {
 ...
 */
export function getCategoryDefinitions(): Record<MpesaCategory, string> {
  return { ...CATEGORY_DESCRIPTIONS };
}

export { MPESA_CATEGORIES, CATEGORY_LABELS };
export type { MpesaCategory };
