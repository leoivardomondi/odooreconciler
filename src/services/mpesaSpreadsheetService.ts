...
import type { ParsedMpesaTransaction } from './mpesaReconciliationService';
...
function inferredTransactionType(transactionTypeText: string, paidIn: number, withdrawn: number) {
  const normalized = transactionTypeText.toLowerCase();
  if (withdrawn > 0 && /charge|fee/.test(normalized)) return 'mpesa_charge';
  if (paidIn > 0) return 'customer_receipt';
  if (withdrawn > 0) return 'outgoing_payment';
  return 'unknown';
}
...
export async function extractMpesaSpreadsheet(input: {
  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    rows.slice(headerIndex + 1).forEach((row) => {
      ...
      if (!receiptNumber && !completion.date && !completion.time) return;
      if (paidInValue <= 0 && withdrawnValue <= 0) return;
