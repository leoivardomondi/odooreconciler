...
function nairobiDateKey(value: Date) {
  return new Intl.DateTimeFormat('en-CA', { timeZone: 'Africa/Nairobi', year: 'numeric', month: '2-digit', day: '2-digit' }).format(value);
}

function isNonWorkingDay(value: Date) {
  return value.getUTCDay() === 0;
}

function workEndUtcHour(value: Date) {
  return value.getUTCDay() === 6 ? NAIROBI_SATURDAY_END_UTC_HOUR : NAIROBI_WORK_END_UTC_HOUR;
}

function nextWorkingStart(value: Date) {
  const result = new Date(value);
  while (isNonWorkingDay(result)) result.setUTCDate(result.getUTCDate() + 1);
  result.setUTCHours(NAIROBI_WORK_START_UTC_HOUR, 0, 0, 0);
  return result;
}

function normalizeWorkingStart(value: Date) {
  const result = new Date(value);
  if (isNonWorkingDay(result)) return nextWorkingStart(result);
  ...
  if (minutes < workStart) result.setUTCHours(NAIROBI_WORK_START_UTC_HOUR, 0, 0, 0);
  if (minutes >= workEnd) { result.setUTCDate(result.getUTCDate() + 1); return nextWorkingStart(result); }
  const activeBreak = BREAKS_UTC_MINUTES.find(([from, to]) => minutes >= from && minutes < to);
  if (activeBreak) result.setUTCHours(Math.floor(activeBreak[1] / 60), activeBreak[1] % 60, 0, 0);
  return result;
}

function addWorkingMinutes(start: Date, totalMinutes: number) {
  ...
  while (remaining > 0) {
    ...
    if (activeBreak) { result.setUTCHours(Math.floor(activeBreak[1] / 60), activeBreak[1] % 60, 0, 0); continue; }
    ...
    remaining -= used;
    if (remaining > 0 && boundaryMinutes === dayEndMinutes) { result.setUTCDate(result.getUTCDate() + 1); result.setTime(nextWorkingStart(result).getTime()); }
  }
  return result;
}

function isEdgingOperation(productName: string | null | undefined) {
  return /edge\s*band|edging/i.test(productName || '');
}

export function getMoEstimatedFinish(input: MoTimingInput) {
...
export function getConfirmedMoQueueSchedule<T extends { id: number; create_date?: string | null; date_start?: string | null; product_qty?: number | null; product_id?: unknown }>(orders: T[]) {
  const resourceAvailable = new Map<'cutting' | 'edging', Date>();
  const schedule = new Map<number, { estimatedStartAt: string; estimatedFinishAt: string; estimatedDurationMinutes: number; timingBasis: string; queuePriority: 'small-mo-priority' | 'standard' }>();
  const product = (order: T) => Array.isArray(order.product_id) ? String(order.product_id[1] || '') : String(order.product_id || '');
  const priority = (order: T) => !isEdgingOperation(product(order)) && Math.max(1, Number(order.product_qty || 1)) < 10 ? 0 : 1;
...
export function getMoOverdueState(input: MoTimingInput, now = new Date()) {
  const created = parseOdooDate(input.createDate) || parseOdooDate(input.plannedStart);
