    ...
    const attendance = operators.map((operator) => ({
        ...
        days: dates.map((date, index) => {
            ...
            const followingRecord = record && !record.check_out
                ? allAttendanceRecords.find((entry) => (Array.isArray(entry.employee_id) ? entry.employee_id[0] : entry.employee_id) === operator.id &&
...
async function renderWeeklyShopFloorReportPdf(reportInput, scope) {
    ...
    const attendanceTotals = report.attendance.reduce((totals, person) => {
        person.days.forEach((day) => { if (day.status === 'Present')
            ...
            totals.noCheckout += 1; if (day.late)
            totals.late += 1; });
        return totals;
    ...
    report.attendance.forEach((person, index) => {
        ...
        if (document.y < 55)
            tableHeader(attCols);
        const present = person.days.filter((day) => day.status === 'Present').length;
        const absent = person.days.filter((day) => day.status === 'Absent').length;
        const noCheckout = person.days.filter((day) => day.status === 'No checkout').length;
        const rate = person.days.length ? Math.round(((present + noCheckout) / person.days.length) * 100) : 0;
        const y = document.y;
        document.rect(42, y, contentWidth, 29).fill(index % 2 ? '#f8fafc' : '#ffffff');
        ...
        document.font('Helvetica').fontSize(6.4).fillColor(muted).text(person.days.map((day) => `${day.date.slice(5)}:${day.status === 'Present' ? 'P' : day.status === 'Absent' ? 'A' : 'NC'}`).join(' '), 456, y + 5, { width: 88, height: 19 });
        document.y = y + 29;
    });
    document.moveDown(.4);
