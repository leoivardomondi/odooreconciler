...
exports.parseJobSummaryPdf = parseJobSummaryPdf;
exports.parseVendorInvoicePdf = parseVendorInvoicePdf;
const pdf_parse_1 = __importDefault(require("pdf-parse"));
const helpers_1 = require("../utils/helpers");
function escapeRegExp(value) {
    return value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}
function insertLineBreakBeforeLabel(text, label, options) {
    const labelPattern = new RegExp(`${escapeRegExp(label)}\\s*:`, 'gim');
    return text.replace(labelPattern, (matchedLabel, offset, source) => {
...
function parseMillimetreValue(line, label) {
...
function normalizeWrappedLabels(text, labels) {
...
function isConfiguredStopHeader(line, stopHeaders) {
    return stopHeaders.some((header) => header.toLowerCase() === line.toLowerCase());
}
function flushCurrentItem(current, items, logs) {
    if (!current) {
        return null;
    }
...
async function parseJobSummaryPdf(fileBuffer, config) {
    ...
    const rawText = (0, helpers_1.normalizeMultilineText)(parsed.text || '');
    if (!rawText.trim()) {
        return {
            items: [],
            sectionFound: false,
    ...
    const stopHeaders = resolvedConfig.stopHeadersCsv
        ...
        .filter(Boolean);
    let productLineRegex;
    try {
        productLineRegex = new RegExp(resolvedConfig.productLinePattern, 'i');
    }
    catch {
        productLineRegex = new RegExp(helpers_1.DEFAULT_PARSER_CONFIG.productLinePattern, 'i');
...
function extractInvoiceLines(rawText) {
    const lines = rawText
        .split('\n')
        .map((line) => line.replace(/\s+/g, ' ').trim())
        .filter(Boolean);
...
async function parseVendorInvoicePdf(fileBuffer) {
    const parsed = await (0, pdf_parse_1.default)(fileBuffer);
    const rawText = (0, helpers_1.normalizeMultilineText)(parsed.text || '');
    const logs = [];
    if (!rawText.trim()) {
    ...
    const grandTotal = parseMoneyValue(firstMatch(rawText, [
    ...
    const untaxedTotal = parseMoneyValue(firstMatch(rawText, [/(?:Total\s+)?Goods\s*:?\s*([\d,]+(?:\.\d{1,2})?)/i]) || undefined);
