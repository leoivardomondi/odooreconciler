...
exports.getShopFloorDueTasksForUser = getShopFloorDueTasksForUser;
exports.sendHourlyShopFloorTaskReminders = sendHourlyShopFloorTaskReminders;
exports.startShopFloorTaskReminderInterval = startShopFloorTaskReminderInterval;
...
const pwaBadgeService_1 = require("./pwaBadgeService");
...
function reminderRecipientType(email) {
...
async function getShopFloorDueTasksForUser(user, channel = 'app') {
    ...
    if (recipientType === 'raphael')
        return [];
    if (recipientType === 'charles') {
        if (channel !== 'email' || !isNairobiWeekdayMorning())
            return [];
        const uploadedToday = await (0, repositories_1.hasMpesaStatementUploadedSince)(nairobiDayStart());
    ...
    const badge = await (0, pwaBadgeService_1.getPwaBadgeBreakdown)(user).catch(() => null);
...
function reminderHtml(user, tasks) {
    const baseUrl = String(env_1.env.APP_BASE_URL || '').replace(/\/$/, '');
...
async function getOperatorMorningSummary(user) {
    const settings = await (0, repositories_1.getSettings)();
    if (!(0, helpers_1.hasOdooConfiguration)(settings))
        return null;
    const client = new odooClient_1.OdooClient(settings.odoo);
    ...
    const [boardLogging, attendance] = await Promise.all([
        (0, repositories_1.getBoardIntakeLoggingReport)(reportDate, reportDate, settings.mail.shopFloorReportingStartDate),
...
function morningSummaryHtml(summary, tasks) {
    const baseUrl = String(env_1.env.APP_BASE_URL || '').replace(/\/$/, '');
...
function isExcludedHourlyReminderRecipient(email) {
    return isNeverShopFloorReminderRecipient(email) || reminderRecipientType(email) === 'raphael';
}
async function sendHourlyShopFloorTaskReminders(recipientOverride = []) {
    if (!isOperatorReminderHour()) {
        return { skipped: true, reason: 'outside_operator_working_hours', sent: 0, usersWithTasks: 0 };
    }
    const hourKey = new Date().toISOString().slice(0, 13);
    if (lastCompletedHour === hourKey)
        return { skipped: true, sent: 0, usersWithTasks: 0 };
    const settings = await (0, repositories_1.getSettings)();
    const approvedShopFloorUsers = (await (0, repositories_1.getApprovedAuthUsers)()).filter((user) => user.active && (reminderRecipientType(user.email) === 'charles'
    ...
    for (const user of users) {
        ...
        if (isExcludedHourlyReminderRecipient(user.email))
            continue;
    ...
    await (0, logService_1.logEvent)('info', 'Hourly shop-floor task reminders completed', { sent, usersWithTasks, approvedUsers: users.length, excluded });
    return { skipped: false, sent, usersWithTasks };
}
let reminderInterval = null;
function startShopFloorTaskReminderInterval() {
    if (reminderInterval)
        return;
