...
exports.OdooClient = exports.OdooClientError = void 0;
exports.isOdooTrafficPaused = isOdooTrafficPaused;
exports.setOdooTrafficPaused = setOdooTrafficPaused;
exports.enqueueOdooRequest = enqueueOdooRequest;
const axios_1 = __importDefault(require("axios"));
const http_1 = __importDefault(require("http"));
const https_1 = __importDefault(require("https"));
const odooHttpsAgent = new https_1.default.Agent({
    keepAlive: false,
    maxSockets: 1,
});
const odooHttpAgent = new http_1.default.Agent({
    keepAlive: false,
    maxSockets: 1,
});
const odooAxios = axios_1.default.create({
...
let isOdooTrafficPausedFlag = env_1.env.ODOO_PAUSE_TRAFFIC === 'true';
function isOdooTrafficPaused() {
    return isOdooTrafficPausedFlag;
}
function setOdooTrafficPaused(paused) {
...
function enqueueOdooRequest(fn) {
    ...
    const nextTask = odooRequestQueue.then(async () => {
        if (isOdooTrafficPaused()) {
            throw new OdooClientError('Odoo API traffic is currently paused by administrative setting (firewall block / rate limit resolution).', 429);
        }
        const now = Date.now();
...
function isCuttingWorkOrderProductName(value) {
    const normalized = normalizeProductName(value);
    return normalized.startsWith('cutting');
}
function isPhysicalInventoryMove(row, locationId) {
    ...
    const referencesPhysicalInventory = reference.includes('physical inventory') ||
...
class OdooClient {
    ...
    attachmentDownloadTimeoutMs = Number(env_1.env.ATTACHMENT_DOWNLOAD_TIMEOUT_MS || env_1.env.REQUEST_TIMEOUT_MS || 60000);
    baseUrl;
    json2BaseUrl;
    ...
    async testConnection() {
        ...
        let versionResponse;
        try {
            versionResponse = await enqueueOdooRequest(() => odooAxios.get(`${this.baseUrl}/web/version`, {
            ...
            if (code === 'ECONNREFUSED') {
                throw new OdooClientError(`Outbound connection refused (ECONNREFUSED) to ${this.baseUrl}. If running on live hosting/cPanel, your web host's firewall (CSF) may be blocking outbound TCP connections on port 443.`, 502, { code, baseUrl: this.baseUrl });
            }
            if (code === 'ENOTFOUND') {
                throw new OdooClientError(`DNS resolution failed (ENOTFOUND) for ${this.baseUrl}. Please check that the URL domain is spelled correctly.`, 502, { code, baseUrl: this.baseUrl });
            }
            if (code === 'ETIMEDOUT') {
                throw new OdooClientError(`Connection timed out (ETIMEDOUT) reaching ${this.baseUrl} after ${this.timeoutMs}ms.`, 504, { code, baseUrl: this.baseUrl });
            }
            throw new OdooClientError(`Network error (${code}) reaching ${this.baseUrl}: ${netErr?.message || 'Failed to connect'}.`, 502, { code, baseUrl: this.baseUrl });
        }
        const latencyMs = Date.now() - startTime;
        if (versionResponse.status < 200 || versionResponse.status >= 300) {
            throw new OdooClientError(`Could not reach ${this.baseUrl}/web/version (HTTP ${versionResponse.status}). Server returned non-200 status.`, versionResponse.status, versionResponse.data);
        }
        const apiKey = this.credentials.apiKey?.trim() || '';
        if (!apiKey) {
            throw new OdooClientError(`No API Key configured. Please paste an Odoo Developer API Key in connection settings.`, 401);
        }
        let userList = [];
        try {
            userList = await this.request('res.users', 'search_read', {
            ...
            throw new OdooClientError(`Odoo API authentication failed (${errMessage}).${hint}`, authErr?.status || 401, authErr?.responseData || null);
        }
        return {
    ...
    async searchSalesOrders(searchTerm, limit = 25, offset = 0) {
        ...
        const domain = targetCompanyId
            ...
            : searchDomain;
        return this.request('sale.order', 'search_read', {
            domain,
            fields: [
                'id',
                'name',
    ...
    async getConfirmedSalesOrdersSince(confirmedFromDate, limit = 50) {
        const targetCompanyId = await this.getTargetCompanyId();
        return this.request('sale.order', 'search_read', {
            domain: [
    ...
    async searchCustomerInvoicesSince(fromDate, limit = 500) {
        const targetCompanyId = await this.getTargetCompanyId();
        const fromDay = fromDate.slice(0, 10);
        return this.request('account.move', 'search_read', {
            domain: [
                ['company_id', '=', targetCompanyId],
    ...
    async searchInboundCustomerPaymentsSince(fromDate, limit = 500) {
        const targetCompanyId = await this.getTargetCompanyId();
        const fromDay = fromDate.slice(0, 10);
        const baseFields = [
            'id',
    ...
    async searchPurchaseOrders(options = {}) {
        ...
        const targetCompanyId = await this.getTargetCompanyId();
        const domain = [['company_id', '=', targetCompanyId]];
        if (options.fromDate) {
        ...
        if (trimmed) {
            domain.push('|', '|', ['name', 'ilike', trimmed], ['partner_ref', 'ilike', trimmed], ['partner_id', 'ilike', trimmed]);
        }
        return this.request('purchase.order', 'search_read', {
            domain,
            fields: [
                'id',
                'name',
    ...
    async getSaleOrder(orderId) {
        const targetCompanyId = await this.getTargetCompanyId();
        const records = await this.request('sale.order', 'read', {
            ids: [orderId],
            fields: [
                'id',
                'name',
        ...
        if (orderCompanyId !== targetCompanyId) {
            throw new OdooClientError(`Sales Order ${orderId} does not belong to ${env_1.env.ODOO_TARGET_COMPANY_NAME}.`);
        }
        return order;
    }
    async getAttachments(orderId) {
        const attachmentFields = ['id', 'name', 'mimetype', 'create_date', 'write_date', 'file_size'];
        const directAttachments = await this.request('ir.attachment', 'search_read', {
            domain: [
        ...
        let chatterAttachments = [];
        try {
            const messages = await this.request('mail.message', 'search_read', {
                domain: [
                    ['model', '=', 'sale.order'],
                    ['res_id', '=', orderId],
                ],
            ...
            if (messageIds.length > 0) {
                chatterAttachments = await this.request('ir.attachment', 'search_read', {
                    domain: [
            ...
            console.warn('[odoo] Could not load chatter attachments for sale.order.', error instanceof Error ? error.message : error);
        }
        const byId = new Map();
    ...
    async getAttachmentsForSaleOrders(orderIds) {
        ...
        try {
            ...
            if (messageIds.length > 0) {
                const chatterAttachments = await this.request('ir.attachment', 'search_read', {
                    domain: [
                        ['res_model', '=', 'mail.message'],
                        ['res_id', 'in', messageIds],
                    ],
    ...
    async downloadAttachment(attachmentId) {
        const records = await this.request('ir.attachment', 'read', {
            ids: [attachmentId],
        ...
        if (!attachment.datas) {
            throw new OdooClientError(`Attachment ${attachment.name} did not return binary data. Confirm the Odoo user has access to ir.attachment datas.`);
        }
        return {
            id: attachment.id,
    ...
    async updateSaleOrder(orderId, data) {
        return this.request('sale.order', 'write', {
            ids: [orderId],
            vals: data,
        });
    }
    async getSaleOrderFields() {
        const response = await this.request('sale.order', 'fields_get', {
    ...
    async readSaleOrderFields(orderId, fieldNames) {
    ...
    async getSaleOrderStockHandoff(orderId, mappings) {
    ...
    async getSaleOrderStockHandoffs(orderIds, mappings) {
        const result = new Map();
        const ids = [...new Set(orderIds.filter((id) => Number.isInteger(id) && id > 0))];
        if (ids.length === 0) {
            return result;
        ...
        const orders = await this.request('sale.order', 'read', {
            ids,
    ...
    async getSaleOrderLines(orderId) {
        return this.request('sale.order.line', 'search_read', {
    ...
    async getManufacturingOrdersBySaleLineId(saleLineId) {
    ...
    async getProductVariant(productId) {
        const products = await this.request('product.product', 'read', {
            ids: [productId],
            fields: ['id', 'name', 'display_name', 'product_tmpl_id'],
        });
        return products[0] || null;
    }
    async getProductVariantsByTemplate(templateId) {
        return this.request('product.product', 'search_read', {
    ...
    async getBomCandidatesForProduct(productId, templateId) {
        const targetCompanyId = await this.getTargetCompanyId();
        return this.request('mrp.bom', 'search_read', {
            domain: [
                ['active', '=', true],
                '|',
                ['company_id', '=', false],
    ...
    async getBomLines(bomId) {
        return this.request('mrp.bom.line', 'search_read', {
            domain: [['bom_id', '=', bomId]],
    ...
    async getStockLocationById(locationId) {
        const locations = await this.request('stock.location', 'read', {
            ids: [locationId],
            fields: ['id', 'name', 'complete_name'],
        });
        return locations[0] || null;
    }
    async findStockLocationsByName(name) {
        return this.request('stock.location', 'search_read', {
    ...
    async getStockQuants(productId, locationId) {
        return this.request('stock.quant', 'search_read', {
            domain: [
                ['product_id', '=', productId],
    ...
    async adjustStockAtLocation(productId, locationId, targetQuantity) {
        const quants = await this.getStockQuants(productId, locationId);
        if (quants.length > 1) {
            throw new OdooClientError(`Multiple stock quants were found for product ${productId} at location ${locationId}.`);
        }
    ...
    async safeUpdateSaleOrder(orderId, payload, availableFields) {
        ...
        const warnings = skippedFields.length > 0
    ...
    async postChatterMessage(orderId, message) {
        return this.request('sale.order', 'message_post', {
            ids: [orderId],
            body: message,
    ...
    async postChatterAlert(orderId, message, partnerIds = []) {
        return this.request('sale.order', 'message_post', {
            ids: [orderId],
            body: message,
            message_type: 'comment',
    ...
    async findUserByLoginOrEmail(loginOrEmail) {
        ...
        const users = await this.request('res.users', 'search_read', {
    ...
    async findUserByNameLoginOrEmail(value) {
        const trimmed = value.trim();
        if (!trimmed) {
            return null;
        }
    ...
    async getModelFields(modelName, fieldNames) {
    ...
    async writeRecord(model, ids, values) {
        return this.request(model, 'write', {
            ids,
            vals: values,
        });
    }
    async setManufacturingOrderPlanDate(moId, planDate) {
        return this.writeRecord('mrp.production', [moId], {
            date_deadline: planDate,
        });
    }
    async searchReadRecords(model, options) {
        return this.request(model, 'search_read', {
    ...
    async readRecords(model, ids, fields) {
        ...
        return this.request(model, 'read', {
            ids,
            fields,
        });
    }
    async postModelChatterMessage(model, recordId, message) {
        return this.request(model, 'message_post', {
            ids: [recordId],
            body: message,
            message_type: 'comment',
            subtype_xmlid: 'mail.mt_note',
    ...
    async callRecordMethod(model, method, ids, values = {}) {
        return this.request(model, method, { ids, ...values });
    }
    async checkModelAccess(model, operation) {
        return this.request(model, 'check_access_rights', {
            operation,
    ...
    async getTargetCompanyIdValue() {
        return this.getTargetCompanyId();
    }
    async getTargetCompanyId() {
        if (this.targetCompanyId) {
            return this.targetCompanyId;
        }
        const companies = await this.request('res.company', 'search_read', {
            domain: [['name', '=', env_1.env.ODOO_TARGET_COMPANY_NAME]],
            fields: ['id', 'name'],
            limit: 1,
        });
        const company = companies[0];
        if (!company) {
            throw new OdooClientError(`Target company "${env_1.env.ODOO_TARGET_COMPANY_NAME}" was not found in Odoo.`);
        }
     ...
     */
    async findEmployeeByWorkEmail(email) {
        const employees = await this.request('hr.employee', 'search_read', {
     ...
     */
    async findEmployeeByUserEmail(email) {
     ...
     */
    async findEmployeeForShopFloorEmail(email) {
    ...
    async findDepartmentByName(name) {
        const departments = await this.request('hr.department', 'search_read', {
     ...
     */
    async getEmployeesByDepartment(departmentId, companyId, includeArchived = false) {
        return this.request('hr.employee', 'search_read', {
            domain: [
     ...
     */
    async getBulkAttendance(employeeIds, dateStr) {
    ...
    async getTodayAttendance(employeeId) {
        ...
        const checkIns = await this.request('hr.attendance', 'search_read', {
            domain: [
                ['employee_id', '=', employeeId],
            ],
     ...
     */
    async getLateCountThisMonth(employeeId) {
        const now = new Date();
     ...
     */
    async getManufacturingOrderComponents(moId) {
        return this.request('stock.move', 'search_read', {
            domain: [
     ...
     */
    async getRelatedPurchaseOrderState(soName) {
        if (!soName)
            return null;
        const orders = await this.request('sale.order', 'search_read', {
            domain: [['name', '=', soName]],
            fields: ['id'],
            limit: 1,
        });
        if (!orders.length)
            return null;
        const pos = await this.request('purchase.order', 'search_read', {
     ...
     */
    async getBulkManufacturingOrderComponents(moIds) {
        if (!moIds || moIds.length === 0)
            return [];
        return this.request('stock.move', 'search_read', {
            domain: [
     ...
     */
    async getBulkRelatedPurchaseOrderStates(soNames) {
        const stateMap = new Map();
        ...
        for (const po of pos) {
            if (po.origin) {
                ...
                const rank = (state) => state === 'purchase' ? 3 : state === 'to approve' ? 2 : state === 'sent' ? 1 : 0;
                if (!existing || rank(po.state) > rank(existing))
                    stateMap.set(po.origin, po.state);
            }
        }
        return stateMap;
    }
    async getManufacturingOrderBoardProcurement(moId) {
        ...
        const unavailableBoards = components.filter((component) => {
            ...
            if (!(0, boardProductClassifier_1.isBoardProductName)(productName) || ['done', 'cancel', 'draft', 'assigned'].includes(component.state)) {
                return false;
            }
     ...
     */
    async getBulkSaleOrderConfirmationDates(soNames) {
    ...
    async getEmployeePayslips(employeeId, limit = 12) {
        return this.request('hr.payslip', 'search_read', {
     ...
     */
    async getPayslipLines(payslipId) {
        return this.request('hr.payslip.line', 'search_read', {
     ...
     */
    async getPayslipName(payslipId) {
        const slips = await this.request('hr.payslip', 'search_read', { domain: [['id', '=', payslipId]], fields: ['name'], limit: 1 });
    ...
    async getPayslipPdf(payslipId) {
        try {
            ...
            const slips = await this.request('hr.payslip', 'search_read', {
                domain: [['id', '=', payslipId]],
            ...
            if (slips.length > 0 && slips[0].document_access_url) {
                ...
                if (match && match[1]) {
                    ...
                    try {
                        const response = await enqueueOdooRequest(() => odooAxios.get(downloadUrl, {
            ...
            const attachments = await this.request('ir.attachment', 'search_read', {
                domain: [
     ...
     * checkout is missing.
     */
    async getFailedCheckouts(employeeId) {
        try {
            const records = await this.request('hr.attendance', 'search_read', {
                domain: [
                    ['employee_id', '=', employeeId],
     ...
     */
    async getWorkCenterPerformance(employeeId, employeeName = '') {
        try {
            const wcDomain = [];
            // 1. Find allowed work centers
            const workcenters = await this.request('mrp.workcenter', 'search_read', { domain: wcDomain, fields: ['id', 'name'], limit: 10 });
            ...
            const reportValue = (type) => reportParts.find((part) => part.type === type)?.value || '';
            ...
            const overdueWorkorders = await this.request('mrp.workorder', 'search_read', {
                domain: [
                    ['workcenter_id', 'in', wcIds],
                    ['state', 'not in', ['done', 'cancel']],
                    ['date_start', '!=', false],
            ...
            const moIds = [...new Set(workorders.filter(w => w.production_id && Array.isArray(w.production_id)).map(w => w.production_id[0]))];
            let moMap = new Map();
            if (moIds.length > 0) {
                const mos = await this.request('mrp.production', 'search_read', {
                    domain: [['id', 'in', moIds]],
            ...
            const overdueWoWeight = (overdueWorkorders || []).reduce((sum, wo) => sum + Math.max(1, Number(wo.qty_production || wo.qty_produced || 0)), 0);
     ...
     */
    async getManufacturingTimingSummary(employeeId, employeeName = '') {
        try {
            const wcDomain = [];
            const workcenters = await this.request('mrp.workcenter', 'search_read', { domain: wcDomain, fields: ['id', 'name'], limit: 10 });
            if (!workcenters || workcenters.length === 0)
     ...
     */
    async getManufacturingTimelineData(employeeId, employeeName = '') {
        try {
            const wcDomain = [];
            const workcenters = await this.request('mrp.workcenter', 'search_read', { domain: wcDomain, fields: ['id', 'name'], limit: 20 });
            if (!workcenters || workcenters.length === 0)
                return null;
            const wcIds = workcenters.map((w) => w.id);
            const now = new Date();
    ...
    async getBoardRegistrationSummary(stockScope) {
        try {
            ...
            const cuttingOrders = await this.request('mrp.production', 'search_read', {
                domain: [
                    ['company_id', '=', targetCompanyId],
                    ['state', 'not in', ['cancel']],
                    ['origin', '!=', false],
            ...
            const originNames = [...new Set(cuttingByOrigin.map((mo) => mo.origin).filter((origin) => Boolean(origin)))];
            const saleOrders = originNames.length > 0
                ? await this.request('sale.order', 'search_read', {
                    domain: [
                        ['company_id', '=', targetCompanyId],
                        ['name', 'in', originNames],
                    ],
            ...
            const purchaseOrders = originNames.length > 0
                ? await this.request('purchase.order', 'search_read', {
                    domain: [
                        ['company_id', '=', targetCompanyId],
            ...
            const purchaseLines = purchaseOrderIds.length > 0
                ? await this.request('purchase.order.line', 'search_read', {
                    domain: [
                ...
                : [];
            const purchaseLinesByOrigin = new Map();
            ...
            const physicalMoves = await this.request('stock.move', 'search_read', {
                domain: [
                    ['company_id', '=', targetCompanyId],
                    ['state', '=', 'done'],
                    ['date', '>=', firstDayStr],
            ...
            const boardPurchasesByOrigin = new Set();
            for (const origin of originNames) {
                const lines = purchaseLinesByOrigin.get(origin) || [];
                if (lines.some((line) => (0, boardProductClassifier_1.isBoardProductName)(Array.isArray(line.product_id) ? line.product_id[1] : ''))) {
    ...
    async request(model, method, payload, options) {
        let attempt = 0;
        try {
            while (true) {
                const response = await enqueueOdooRequest(() => odooAxios.post(`${this.json2BaseUrl}/${model}/${method}`, payload, {
                ...
                throw new OdooClientError(this.extractErrorMessage(response.data, response.status), response.status, response.data);
            }
        }
        catch (error) {
            if (error instanceof OdooClientError) {
            ...
            if (axios_1.default.isAxiosError(error)) {
                throw new OdooClientError(this.extractAxiosMessage(error, model, method), error.response?.status, {
                    code: error.code,
                    model,
                    method,
                });
    ...
    async authenticateWebSession(force = false) {
        ...
        const response = await enqueueOdooRequest(() => odooAxios.post(`${this.baseUrl}/web/session/authenticate`, {
            jsonrpc: '2.0',
            method: 'call',
            params: {
        ...
        if (!response.data?.result?.uid) {
            if (responseCookie) {
                const sessionProbe = await enqueueOdooRequest(() => odooAxios.post(`${this.baseUrl}/web/session/get_session_info`, { jsonrpc: '2.0', method: 'call', params: {}, id: ++this.webRpcSequence }, {
                    headers: {
                        'Content-Type': 'application/json; charset=utf-8',
                        Cookie: responseCookie,
                        'User-Agent': 'odoo-job-summary-extractor-node',
    ...
    async beginShopFloorSession() {
        ...
        this.webSessionContext = {};
        const loginPage = await enqueueOdooRequest(() => odooAxios.get(`${this.baseUrl}/web/login`, {
        ...
        const loginResponse = await enqueueOdooRequest(() => odooAxios.post(`${this.baseUrl}/web/login`, loginBody, {
            headers: {
        ...
        if (/\/web\/login\/totp/i.test(location)) {
            const otpPage = await enqueueOdooRequest(() => odooAxios.get(new URL(location, this.baseUrl).toString(), {
                headers: {
                    Cookie: loginCookie,
                    'User-Agent': 'odoo-job-summary-extractor-node',
                },
        ...
        const sessionInfoResponse = await enqueueOdooRequest(() => odooAxios.post(`${this.baseUrl}/web/session/get_session_info`, { jsonrpc: '2.0', method: 'call', params: {}, id: ++this.webRpcSequence }, {
            headers: {
                'Content-Type': 'application/json; charset=utf-8',
                Cookie: loginCookie,
                'User-Agent': 'odoo-job-summary-extractor-node',
    ...
    async verifyShopFloorOtp(code) {
        ...
        const response = await enqueueOdooRequest(() => odooAxios.post(`${this.baseUrl}/web/login/totp`, formBody, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
    ...
    async callNativeShopFloorMethod(model, method, args, kwargs = {}, retrySession = true) {
        ...
        const response = await enqueueOdooRequest(() => odooAxios.post(`${this.baseUrl}/web/dataset/call_kw/${encodeURIComponent(model)}/${encodeURIComponent(method)}`, {
            jsonrpc: '2.0',
            method: 'call',
            params: {
                model,
     ...
     */
    async getTeamPenalties(stockScope) {
        try {
            ...
            if (scopedWarehouseId > 0) {
                ...
                const scopedPickings = await this.searchReadRecords('stock.picking', {
            ...
            const pickings = await this.request('stock.picking', 'search_read', {
            ...
            if (pickingTypeId) {
                const receiptCounters = await this.request('stock.picking.type', 'search_read', {
                ...
                if (warehouseId > 0) {
                    const warehouses = await this.request('stock.warehouse', 'search_read', {
    ...
    extractAxiosMessage(error, model, method) {
        if (error.response) {
    ...
    async getMaintenanceEquipment() {
        return this.searchReadRecords('maintenance.equipment', {
            domain: [],
     ...
     */
    async getMaintenanceRequests(limit = 50) {
        return this.searchReadRecords('maintenance.request', {
            domain: [],
            fields: [
                'id',
                'name',
     ...
     */
    async resolveMaintenanceRequest(id) {
        const stages = await this.searchReadRecords('maintenance.stage', {
     ...
     */
    async getEmployeeAssignedEquipment(employeeId) {
        return this.searchReadRecords('maintenance.equipment', {
            domain: [['employee_id', '=', employeeId]],
     ...
     */
    async getBulkAssignedEquipment() {
        return this.searchReadRecords('maintenance.equipment', {
     ...
     */
    async assignEquipmentToEmployee(email, itemName, assignDate) {
        try {
            ...
            const equipments = await this.searchReadRecords('maintenance.equipment', {
    ...
    async getBoardProducts(warehouseId) {
        let categoryId = null;
        try {
            const categories = await this.searchReadRecords('product.category', {
        ...
        const products = await this.searchReadRecords('product.product', {
            domain,
     ...
     */
    async getOdooCustomers(limit = 500) {
     ...
     */
    async searchPartners(searchTerm, limit = 500) {
        const trimmed = String(searchTerm || '').trim();
        const domain = [];
        if (trimmed) {
            domain.push('|', '|', '|', ['name', 'ilike', trimmed], ['email', 'ilike', trimmed], ['phone', 'ilike', trimmed], ['ref', 'ilike', trimmed]);
        }
        return this.searchReadRecords('res.partner', {
            domain,
    ...
    async getPartnerById(partnerId) {
        ...
        const partners = await this.readRecords('res.partner', [partnerId], ['id', 'name']);
        return partners[0] || null;
    }
    /**
     * Get active board reservations from Odoo (represented as draft/waiting internal transfers).
     */
    async getActiveBoardReservations() {
        try {
     ...
     */
    async getMainWarehouseLocationId() {
        const targetCompanyId = await this.getTargetCompanyId();
        const warehouses = await this.searchReadRecords('stock.warehouse', {
        ...
        const locations = await this.searchReadRecords('stock.location', {
            domain: [['usage', '=', 'internal']],
            fields: ['id'],
            limit: 1,
        });
        if (!locations.length)
            throw new OdooClientError('No stock location found in Odoo.');
        return locations[0].id;
    }
    async ensureBoardProductIsStockable(productId) {
        const variants = await this.request('product.product', 'read', {
            ids: [productId],
        ...
        const availableFields = await this.request('product.template', 'fields_get', {
            attributes: ['type', 'readonly'],
        });
        const fields = ['id', 'name'];
        if (availableFields.is_storable)
            fields.push('is_storable');
        if (availableFields.detailed_type)
            fields.push('detailed_type');
        if (availableFields.type)
            fields.push('type');
        const templates = await this.request('product.template', 'read', {
            ids: [templateId],
            fields,
        });
        const template = templates[0];
        if (!template)
            throw new OdooClientError(`Odoo product template ${templateId} was not found.`);
        const alreadyStockable = template.is_storable === true ||
            template.detailed_type === 'product' ||
            template.type === 'product';
        if (alreadyStockable)
            return { changed: false, productName, templateId };
        if (availableFields.is_storable) {
            ...
            throw new OdooClientError(`${productName} is a consumable and this Odoo version exposes no writable stockable-product field.`);
        }
    ...
    async removeBoardsFromStock(input) {
        ...
        const quants = await this.getStockQuants(input.productId, locationId);
        if (!quants.length)
            throw new OdooClientError('No Odoo stock record exists for this board, so the intake cannot be reverted.');
    ...
    async assertPickingBelongsToWarehouse(pickingId, warehouseId, operation) {
        const warehouse = await this.getVerifiedTargetWarehouse(warehouseId);
        const records = await this.searchReadRecords('stock.picking', {
     ...
     */
    async populateInventoryAdjustmentOperator(productId, operatorName) {
        if (!Number.isFinite(productId) || productId <= 0 || !operatorName.trim())
            return false;
        try {
            ...
            const lines = await this.searchReadRecords('stock.move.line', {
    ...
    async validateDelivery(pickingId, actorEmail, warehouseId) {
    ...
    async advanceManufacturingOrder(moId, action, employeeId) {
        ...
        const assertEmployeeMayUseWorkcenter = async (workorder) => {
            ...
            const workcenters = await this.searchReadRecords('mrp.workcenter', {
            ...
            if (allowedEmployees.length && !allowedEmployees.includes(employeeId)) {
                throw new OdooClientError(`You are not assigned to ${workorder.workcenter_id[1]} in Odoo. Ask an administrator to add you under Allowed Employees.`);
            }
        };
        if (action === 'start') {
    ...
    async findMOsForBoardIntake(input) {
        try {
            ...
            const moIdsNeedingBoard = new Set(matchingMoves.map(m => {
            ...
            const soOriginsWithBoardPO = new Set();
            if (purchaseOrders.length > 0) {
                ...
                for (const line of poLines) {
                    ...
                    if (productId === input.productId) {
                        ...
                        if (origin)
                            soOriginsWithBoardPO.add(origin);
                    }
                }
            }
