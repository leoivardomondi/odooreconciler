...
function computeVendorScore(invoiceVendor, poVendor) {
...
function buildCandidate(purchaseOrder, poLines, parsedInvoice) {
...
async function registerPaymentForVendorBill(client, vendorBill, pinNote, parsedInvoiceDate) {
    const paymentDate = vendorBill.invoice_date ||
    ...
    try {
        ...
        console.warn(`[po-bills] account.payment.register wizard failed for bill ${vendorBill.id}, attempting direct account.payment:`, wizardError instanceof Error ? wizardError.message : wizardError);
    }
    try {
        const paymentValues = {
            payment_type: 'outbound',
...
async function runPoBillAutomation(client, input) {
    ...
    const acceptableCandidate = candidates.find((candidate) => isConfirmedPurchaseOrderState(candidate.purchaseOrder.state) ||
        isApprovablePurchaseOrderState(candidate.purchaseOrder.state));
    ...
    let matchedPurchaseOrders = combinedMatch?.purchaseOrders ||
        (overridePurchaseOrder || orderNumberPurchaseOrder || acceptableCandidate?.purchaseOrder
            ? [overridePurchaseOrder || orderNumberPurchaseOrder || acceptableCandidate?.purchaseOrder].filter(Boolean)
            : []);
    ...
    addCheck(checks, 'Core Match', coreMatchPassed ? 'pass' : 'fail', `Vendor, total due, and date scored ${coreMatchScore}/95. Auto mode requires ${CORE_MATCH_THRESHOLD}/95 or better.`);
