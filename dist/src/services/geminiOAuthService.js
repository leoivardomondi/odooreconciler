...
exports.getGeminiOAuthRedirectUri = getGeminiOAuthRedirectUri;
exports.createGeminiOAuthState = createGeminiOAuthState;
exports.verifyGeminiOAuthState = verifyGeminiOAuthState;
exports.buildGeminiOAuthAuthorizationUrl = buildGeminiOAuthAuthorizationUrl;
exports.completeGeminiOAuth = completeGeminiOAuth;
exports.getGeminiOAuthAccessToken = getGeminiOAuthAccessToken;
...
async function requireOAuthClient() {
    const stored = await (0, repositories_1.getGeminiOAuthClientConfig)();
    const clientId = stored?.clientId.trim() || env_1.env.GOOGLE_GEMINI_OAUTH_CLIENT_ID.trim();
    const clientSecret = stored?.clientSecret.trim() || env_1.env.GOOGLE_GEMINI_OAUTH_CLIENT_SECRET.trim();
    if (!clientId || !clientSecret) {
        throw new Error('Gemini Google OAuth is not configured. Set GOOGLE_GEMINI_OAUTH_CLIENT_ID and GOOGLE_GEMINI_OAUTH_CLIENT_SECRET first.');
    }
    return { clientId, clientSecret };
}
function getGeminiOAuthRedirectUri() {
    const baseUrl = env_1.env.APP_BASE_URL.trim().replace(/\/+$/, '');
    if (!baseUrl) {
        throw new Error('APP_BASE_URL is required for the Gemini Google OAuth callback.');
    }
    return `${baseUrl}/settings/ai/gemini/callback`;
}
function createGeminiOAuthState(sessionId) {
    if (!sessionId)
        throw new Error('An authenticated app session is required to connect Gemini.');
    const payload = Buffer.from(JSON.stringify({
        sessionId,
        issuedAt: Date.now(),
        nonce: crypto_1.default.randomBytes(18).toString('hex'),
    })).toString('base64url');
    const signature = crypto_1.default.createHmac('sha256', oauthSecret()).update(payload).digest('base64url');
    return `${payload}.${signature}`;
}
function verifyGeminiOAuthState(state, sessionId) {
    if (!state || !sessionId)
        return false;
    const [payload, signature] = state.split('.');
    if (!payload || !signature)
        return false;
    const expected = crypto_1.default.createHmac('sha256', oauthSecret()).update(payload).digest('base64url');
...
async function buildGeminiOAuthAuthorizationUrl(input) {
...
async function loadGoogleEmail(accessToken) {
    try {
...
async function completeGeminiOAuth(input) {
    ...
    await (0, repositories_1.saveGeminiOAuthCredentials)({
        refreshToken,
...
async function getGeminiOAuthAccessToken() {
    const credentials = await (0, repositories_1.getGeminiOAuthCredentials)();
    if (!credentials) {
        throw new Error('Google Gemini OAuth is not connected.');
    }
    const settings = await (0, repositories_1.getSettings)();
    const projectId = settings.ai.geminiOAuth.projectId.trim() || env_1.env.GOOGLE_GEMINI_PROJECT_ID.trim();
    if (!projectId) {
    ...
    await (0, repositories_1.updateGeminiOAuthAccessToken)({ accessToken, accessTokenExpiresAt });
