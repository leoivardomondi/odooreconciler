...
router.get('/extractions/:historyId', async (req, res) => {
    ...
    try {
        const model = await (0, extractionService_1.getExtractionViewModel)(historyId);
        ...
        const details = caughtError instanceof Error ? caughtError.message : 'Could not load the extraction.';
...
router.post('/extractions/:historyId/compare-signature', async (req, res) => {
    const { historyId } = req.params;
    try {
        const result = await (0, extractionService_1.compareExtractionSignature)(historyId);
        const message = result.comparisonResult === 'match'
...
router.post('/extractions/:historyId/send-to-odoo', async (req, res) => {
    ...
    try {
        const result = await (0, extractionService_1.sendExtractedResultToOdoo)(historyId, forceSend);
        res.redirect(`/extractions/${historyId}?${result.warningMessage
