...
async function autoMatchCategoriesOnGridSave(transactions, patches) {
    ...
    for (const patch of patches) {
        ...
        const isCategoryUnset = !userCategorySubmitted ||
...
function buildStatementTotalChecks(rawTextPreview, transactions) {
    ...
    const rowChecks = transactions.reduce((result, transaction, index) => {
    ...
    return [
        ...
        const status = !hasSummaryValue
            ? rowsVerified ? 'verified' : 'needs_review'
        ...
        return {
            ...
            difference,
            status,
            validationMode: hasSummaryValue ? 'summary' : 'rows',
...
router.post('/mpesa-reconciliation/batches/:batchId/categorize-from-notes', async (req, res) => {
    ...
    try {
        ...
        const notesMap = req.body.notesByTransactionId && typeof req.body.notesByTransactionId === 'object'
            ? req.body.notesByTransactionId
            : {};
        const targetIds = Array.isArray(req.body.transactionIds)
            ? req.body.transactionIds.filter((id) => typeof id === 'string')
            : [];
        ...
        for (const { transaction: tx, effectiveNote } of eligibleTransactions) {
            const matchResult = await (0, aiCategoryService_1.categorizeWithAi)({
                details: tx.details,
                counterparty: tx.counterparty,
                direction: tx.direction,
                paidIn: tx.paidIn,
...
async function matchTransactionsByNotesPoRef(transactions, odooClient, batchId) {
    ...
    for (const tx of transactions) {
        ...
        if (exactPos.length === 1 && tx.matchedPoId === exactPos[0].id)
            continue;
...
router.post('/mpesa-reconciliation/batches/:batchId/auto-match', async (req, res) => {
    ...
    try {
        ...
        res.redirect(`/mpesa-reconciliation?batch=${encodeURIComponent(batch.id)}&message=${encodeURIComponent(`Updated candidates for ${totalCandidates} transaction(s) (${extractionCandidatesCount} from extraction, ${notesCandidatesCount} from notes). POs are left unselected for manual confirmation.`)}`);
    }
    catch (error) {
