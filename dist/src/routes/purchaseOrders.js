...
async function buildPurchaseOrderListItems(orderSummaries) {
    const processedByPoId = await (0, repositories_1.getLatestPoBillProcessedDocumentsByPurchaseOrderIds)(orderSummaries.map((order) => order.id));
...
router.get('/purchase-orders', async (req, res) => {
    ...
    const shouldLoad = req.query.load === '1' || query.length > 0 || hasExplicitFromDate || hasExplicitToDate || page > 1;
    if (!shouldLoad) {
