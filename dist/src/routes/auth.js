...
function appendQueryMessage(targetPath, message) {
...
router.post('/auth/login', async (req, res) => {
    ...
    const nextPath = (0, authService_1.getSafeRedirectPath)(req);
    const requestContext = (0, authService_1.getRequestContext)(req);
    try {
        if (!credential) {
            await (0, authService_1.requestLoginCode)({
                email,
        ...
        const verified = /^\d{6}$/.test(credential)
            ? await (0, authService_1.verifyLoginCode)({
                email,
                otpCode: credential,
                ipAddress: requestContext.ipAddress,
                userAgent: requestContext.userAgent,
            })
            : await (0, authService_1.verifyLocalPasswordLogin)({
                email,
                password: credential,
                ipAddress: requestContext.ipAddress,
                userAgent: requestContext.userAgent,
...
router.get('/login', (req, res) => {
    if (req.authUser) {
        return res.redirect((0, authService_1.getSafeRedirectPath)(req));
    }
    return renderLoginPage(res, {
        status: typeof req.query.message === 'string'
            ? { type: 'success', message: req.query.message }
            : null,
        next: (0, authService_1.getSafeRedirectPath)(req),
    });
});
...
router.post('/auth/request-password-reset', async (req, res) => {
    ...
    try {
        ...
        await (0, authService_1.requestLoginCode)({ email, redirectPath: '/forgot-password', ipAddress: context.ipAddress });
...
router.post('/auth/reset-password', async (req, res) => {
    ...
    try {
        ...
        const user = await (0, repositories_1.getApprovedAuthUserByEmail)(email);
        if (!user || !user.active)
            throw new Error('This account is not active.');
        await (0, repositories_1.upsertApprovedAuthUser)(user.email, user.role, user.apps || [], true, await (0, authService_1.hashApprovedUserPassword)(password));
        await (0, authService_1.recordAuthLoginEvent)({
            email: user.email,
            role: user.role,
...
router.post('/auth/verify-code', async (req, res) => {
    ...
    try {
        const requestContext = (0, authService_1.getRequestContext)(req);
        const verified = await (0, authService_1.verifyLoginCode)({
            email,
            otpCode: String(req.body.otpCode || ''),
            ipAddress: requestContext.ipAddress,
            userAgent: requestContext.userAgent,
...
router.post('/auth/password-login', async (req, res) => {
    ...
    try {
        const requestContext = (0, authService_1.getRequestContext)(req);
        const verified = await (0, authService_1.verifyLocalPasswordLogin)({
            email,
