...
function encode(payload) { return `${MARKER}${Buffer.from(JSON.stringify(payload)).toString('base64')}`; }
function decode(value) {
    ...
    if (index < 0)
        return null;
    try {
        return JSON.parse(Buffer.from(source.slice(index + MARKER.length).split(/\s|</)[0], 'base64').toString('utf8'));
    }
    catch {
        return null;
    }
}
async function client() { const settings = await (0, repositories_1.getSettings)(); return new odooClient_1.OdooClient(settings.odoo); }
async function supportedValues(odoo, model, values) {
    const fields = await odoo.getModelFields(model, Object.keys(values));
...
async function relationId(odoo, model, name) {
    if (!name)
        return null;
    const rows = await odoo.searchReadRecords(model, { domain: [['name', 'ilike', name]], fields: ['id'], limit: 1 });
    return rows[0]?.id || null;
}
async function syncApplicantToOdoo(odoo, payload) {
    const partnerName = `${PREFIX} ${payload.fullName}`;
    const existing = await odoo.searchReadRecords('hr.applicant', {
...
router.post('/staff-onboarding', async (req, res) => {
    let localId = '';
    try {
        ...
        if (Date.now() - previous < 60_000) {
            res.redirect('/staff-onboarding?error=' + encodeURIComponent('Please wait one minute before submitting again.'));
            return;
        }
        const payload = {};
        ...
        if (!payload.fullName || !payload.personalEmail || !payload.mobilePhone || !payload.identificationId)
            throw new Error('Full name, personal email, mobile phone and national ID are required.');
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payload.personalEmail))
            throw new Error('Enter a valid personal email address.');
        localId = (0, uuid_1.v4)();
        await (0, repositories_1.createStaffOnboardingApplication)({ id: localId, payload });
        recentSubmissions.set(ip, Date.now());
        const odoo = await client();
        const applicantId = await syncApplicantToOdoo(odoo, payload);
        await (0, repositories_1.updateStaffOnboardingSync)(localId, applicantId);
        await (0, logService_1.logEvent)('info', 'Public staff onboarding submitted', { applicantId, name: payload.fullName });
        res.redirect('/staff-onboarding?submitted=1');
    }
    catch (error) {
        const message = error instanceof Error ? error.message : 'Could not submit onboarding form.';
        if (localId) {
            await (0, repositories_1.updateStaffOnboardingSync)(localId, null, message).catch(() => undefined);
            res.redirect('/staff-onboarding?submitted=1');
            return;
        }
...
router.get('/settings/staff-onboarding', async (req, res) => {
    if (req.authUser?.role !== 'admin') {
        res.status(403).send('Administrator access required.');
        return;
    }
    const applications = await (0, repositories_1.getStaffOnboardingApplications)();
...
router.post('/settings/staff-onboarding/import', async (req, res) => {
    if (req.authUser?.role !== 'admin') {
        res.status(403).send('Administrator access required.');
        return;
    }
    try {
        const odoo = await client();
        const rows = await odoo.searchReadRecords('hr.applicant', {
        ...
        for (const row of rows) {
            ...
            if (!payload?.fullName || !payload.personalEmail)
                continue;
            await (0, repositories_1.importStaffOnboardingApplication)({ id: (0, uuid_1.v4)(), payload, odooApplicantId: row.id });
            imported += 1;
        }
...
router.post('/settings/staff-onboarding/:id/sync', async (req, res) => {
    if (req.authUser?.role !== 'admin') {
        res.status(403).send('Administrator access required.');
        return;
    }
    const application = await (0, repositories_1.getStaffOnboardingApplication)(req.params.id);
    if (!application || application.odooApplicantId) {
        res.redirect('/settings/staff-onboarding');
        return;
    }
    try {
        const odoo = await client();
        ...
        const applicantId = await syncApplicantToOdoo(odoo, payload);
        await (0, repositories_1.updateStaffOnboardingSync)(application.id, applicantId);
        ...
        await (0, repositories_1.updateStaffOnboardingSync)(application.id, null, message);
        res.redirect('/settings/staff-onboarding?error=' + encodeURIComponent(message));
    }
});
router.post('/settings/staff-onboarding/:id/approve', async (req, res) => {
    if (req.authUser?.role !== 'admin' || !req.authUser) {
        res.status(403).send('Administrator access required.');
        return;
    }
    const application = await (0, repositories_1.getStaffOnboardingApplication)(req.params.id);
    if (!application) {
        res.redirect('/settings/staff-onboarding?error=' + encodeURIComponent('Application not found.'));
        return;
    }
    if (!application.odooApplicantId) {
        res.redirect('/settings/staff-onboarding?error=' + encodeURIComponent('Synchronize this application with Odoo before approval.'));
        return;
    }
    if (!(await (0, repositories_1.beginStaffOnboardingApproval)(application.id))) {
        res.redirect('/settings/staff-onboarding?error=' + encodeURIComponent('This application is already approved or being processed.'));
        return;
    }
    ...
    try {
        ...
        await (0, repositories_1.finishStaffOnboardingApproval)(application.id, { employeeId, reviewedBy: reviewer });
        if (payload.bankAccount) {
            try {
                const employee = await odoo.searchReadRecords('hr.employee', { domain: [['id', '=', employeeId]], fields: ['work_contact_id', 'private_address_id'], limit: 1 });
                ...
                if (partner)
                    await odoo.createRecord('res.partner.bank', await supportedValues(odoo, 'res.partner.bank', { partner_id: partner, bank_id: bankId, acc_number: payload.bankAccount, acc_holder_name: payload.fullName }));
            }
            catch (bankError) {
                await (0, logService_1.logEvent)('error', 'Approved employee bank record follow-up failed', { applicationId: application.id, employeeId, error: bankError instanceof Error ? bankError.message : String(bankError) }).catch(() => undefined);
            }
        }
        await odoo.writeRecord('hr.applicant', [application.odooApplicantId], { partner_name: `[APPROVED ${employeeId}] ${payload.fullName}` }).catch((applicantError) => (0, logService_1.logEvent)('error', 'Approved applicant label follow-up failed', { applicationId: application.id, employeeId, error: applicantError instanceof Error ? applicantError.message : String(applicantError) }).catch(() => undefined));
        await (0, logService_1.logEvent)('info', 'Staff onboarding approved and Odoo employee created', { applicationId: application.id, employeeId, approvedBy: reviewer, approvedByEmail: req.authUser.email });
        ...
        const latest = await (0, repositories_1.getStaffOnboardingApplication)(application.id);
        if (latest?.status !== 'approved')
            await (0, repositories_1.finishStaffOnboardingApproval)(application.id, { reviewedBy: reviewer, errorMessage: message });
        res.redirect('/settings/staff-onboarding?error=' + encodeURIComponent(message));
    }
});
