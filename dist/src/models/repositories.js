...
function buildGeminiOAuthStatus(rawAi, encryptedKeysPayload) {
    ...
    const clientId = client?.clientId || String(rawStatus.clientId || '') || env_1.env.GOOGLE_GEMINI_OAUTH_CLIENT_ID.trim();
    const hasClientSecret = Boolean(client?.clientSecret || env_1.env.GOOGLE_GEMINI_OAUTH_CLIENT_SECRET.trim());
    return {
...
async function saveSettings(input) {
    ...
    const nextGeminiClientId = submittedAi.clearGeminiOAuthClientId
        ? ''
        : submittedGeminiClientId || existingGeminiOAuthClient?.clientId || env_1.env.GOOGLE_GEMINI_OAUTH_CLIENT_ID.trim();
    const nextGeminiClientSecret = submittedAi.clearGeminiOAuthClientSecret
        ? ''
        : submittedGeminiClientSecret || existingGeminiOAuthClient?.clientSecret || env_1.env.GOOGLE_GEMINI_OAUTH_CLIENT_SECRET.trim();
    ...
    else if (!submittedAi.clearGeminiOAuthClientId &&
...
async function getRecentBoardIntakeQueueEntries(limit = 12, offset = 0) {
    ...
    LIMIT ? OFFSET ?`;
    try {
        return await (0, db_1.queryAll)(query, [Math.max(1, Math.min(50, limit)), Math.max(0, offset)]);
    }
    catch (_error) {
        ...
        return (0, db_1.queryAll)(query, [Math.max(1, Math.min(50, limit)), Math.max(0, offset)]);
    }
}
...
async function getPendingShopFloorProcessesCount(status = 'pending') {
    const row = await (0, db_1.queryOne)(`SELECT COUNT(*) as count FROM shop_floor_pending_processes 
     WHERE status = ? AND mo_name NOT LIKE 'VA/%' AND partner_name != 'URBAN VIBE 2' AND partner_id != 350`, [status]);
    return Number(row?.count || 0);
}
