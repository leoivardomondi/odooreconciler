...
exports.normalizeInvoiceTotals = normalizeInvoiceTotals;
const extractTotals_1 = require("./extractTotals");
...
function nearlyEqualComputedMoney(left, right) {
    return (0, normalizeMoney_1.nearlyEqualMoney)(left, right, Math.max(2, Math.abs(right) * 0.03));
}
...
function chooseMostLikelyMoney(values) {
    if (values.length === 0) {
        return null;
    }
...
function inferGoodsFromGrossAndVat(gross, vat) {
    if (gross === null || vat === null || gross <= vat) {
        return null;
    }
    return roundMoney(gross - vat);
}
function findExplicitAmountDueCandidates(text) {
    const values = [];
    const patterns = [
...
function findStrongFinalTotal(text) {
    const patterns = [
        /\bTOTAL\s*\(\s*KES\s*\)\s*([\d\s,.]+)(?=\s|$)/gi,
...
function findWeakPaymentTotal(text) {
    const values = [];
    const patterns = [
...
function findLabelledAmountDue(text) {
...
function sumLineNetAmounts(invoice) {
...
function sumClearLineItems(invoice) {
    const values = invoice.items
        .filter((item) => {
...
function inferKenyaVatInclusiveTotal(vat) {
    if (vat === null) {
        return null;
    }
...
function findVatConsistentMoney(text, vat) {
    const targetGross = inferKenyaVatInclusiveTotal(vat);
    if (targetGross === null) {
        return null;
    }
    const values = [...text.matchAll(/\b(\d[\d\s,]*(?:[.,]\s*\d{1,3})?)\b/g)]
        .map((match) => (0, normalizeMoney_1.normalizeMoney)(match[1].replace(/\s+/g, '')))
...
function normalizeInvoiceTotals(invoice) {
    ...
    const currentLooksLikeWeakPayment = currentAmountDue !== null &&
        ...
        (0, normalizeMoney_1.nearlyEqualMoney)(currentAmountDue, weakPaymentTotal, 1) &&
    ...
    const currentLooksUntaxed = currentAmountDue !== null &&
        goodsTotal !== null &&
        (0, normalizeMoney_1.nearlyEqualMoney)(currentAmountDue, goodsTotal, 1) &&
        computedAmountDue !== null &&
        computedAmountDue > currentAmountDue;
    const currentInconsistent = currentAmountDue !== null &&
        goodsTotal !== null &&
        vat !== null &&
        !(0, normalizeMoney_1.nearlyEqualMoney)(goodsTotal + vat, currentAmountDue, 2);
    const vatInferenceLooksBetter = vatInferredAmountDue !== null &&
    ...
    const labelledAmountDueLooksImplausible = labelledAmountDue !== null &&
        vatInferredAmountDue !== null &&
        labelledAmountDue > vatInferredAmountDue * 1.25 &&
        strongFinalTotal === null;
    const currentLooksLikeVat = currentAmountDue !== null &&
        vat !== null &&
    ...
    const changed = recovered.goods_total !== invoice.totals?.goods_total ||
